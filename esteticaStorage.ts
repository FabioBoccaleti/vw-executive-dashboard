...
const KEY = 'estetica_audi_rows';

export interface EsteticaRow {
  id: string;
  dataRegistro: string;
...
export function recalcEsteticaRow(row: EsteticaRow, totalAliquotaPct = 0, comissaoCtx?: ComissaoContext): EsteticaRow {
  ...
  if (comissaoCtx) {
    ...
    row.comissaoVendedorAcessorios = row.vendedorAcessorios?.trim()
      ? calcComissaoPorCargo('Vendedor de AcessÃ³rios', lb, rl, comissaoCtx.regras)
      : '0';
  }
  // SituaÃ§Ã£o automÃ¡tica
...
export async function loadEsteticaRows(): Promise<EsteticaRow[]> {
  try {
    const data = await kvGet<EsteticaRow[]>(KEY);
    if (data) return data;
  } catch { /* fallback */ }
  return [];
}

export async function saveEsteticaRows(rows: EsteticaRow[]): Promise<boolean> {
  try {
