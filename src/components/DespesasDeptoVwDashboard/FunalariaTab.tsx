...
export function FunalariaTab({ year, month }: Props) {
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsFunilaria(year, month, newObs);
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          ...
          <p className="text-xs text-slate-400 mt-0.5">Funilaria — Sorana VW</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
      ...
      <div className="bg-slate-800 rounded-xl px-5 py-3.5 flex items-center justify-between shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {periodoLabel} · Funilaria · {totalContas} conta{totalContas !== 1 ? 's' : ''} em {activeGroups.length} grupo{activeGroups.length !== 1 ? 's' : ''}
          </p>
        </div>
