...
export function AdministracaoTab({ year, month }: Props) {
    ...
    const load = async () => {
      ...
      if (isYearMode) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          if (!data) continue;
          const mMap = extractByDeptoRule(data, 'administracao', regras);
        ...
        const [data, obsData] = await Promise.all([
          ...
          loadObsAdministracao(year, month),
        ]);
        valMap = data ? extractByDeptoRule(data, 'administracao', regras) : new Map();
        mergeAssistenciaMedica(valMap);
        mergeSalariosOrdenados(valMap);
        mergeComissoesEmpregados(valMap);
        mergeHorasExtras(valMap);
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsAdministracao(year, month, newObs);
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          ...
          <p className="text-xs text-slate-400 mt-0.5">Administração — Sorana VW</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
      ...
      <div className="bg-slate-800 rounded-xl px-5 py-3.5 flex items-center justify-between shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {periodoLabel} · Administração · {totalContas} conta{totalContas !== 1 ? 's' : ''} em {activeGroups.length} grupo{activeGroups.length !== 1 ? 's' : ''}
          </p>
        </div>
