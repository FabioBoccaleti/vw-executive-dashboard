...
function buildMonthHTML(
  ...
  let html = `<div class="header" style="background:${tabColor}"><h1>Despesas por Departamento na Sorana VW &ndash; ${tabLabel}</h1><h2>${tabLabel} — ${periodoLabel}</h2></div><div class="content">`;
...
function buildYearHTML(
  ...
  let html = `<div class="header" style="background:${tabColor}"><h1>Despesas por Departamento na Sorana VW &ndash; ${tabLabel}</h1><h2>${tabLabel} — Ano ${year} — Evolução Mensal</h2></div><div class="content">`;
...
export async function printDespesasDeptoVw(year: number, month: number): Promise<void> {
...
<head>
  <meta charset="UTF-8"/>
  <title>Despesas por Departamento na Sorana VW — ${year}</title>
  <style>
