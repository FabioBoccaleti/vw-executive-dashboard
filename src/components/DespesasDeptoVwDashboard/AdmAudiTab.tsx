      ...
      <table className="table-fixed w-full text-xs">
        ...
        <tfoot>
          <tr style={{ backgroundColor: cfg.badge }}>
            ...
            <td className="px-4 py-2 text-right text-xs font-bold tabular-nums" style={{ color: cfg.color }}>
              {fmtBRL(subtotal)}
            </td>
            {!isYearMode && <td />}
          </tr>
...
export function AdmAudiTab({ year, month }: Props) {
    ...
    const load = async () => {
      ...
      if (isYearMode) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          if (!data) continue;
          const mMap = extractByDeptoRule(data, 'veiculos_usados', regras);
        ...
        const [data, obsData] = await Promise.all([
          ...
          loadObsAudi(year, month),
        ]);
        valMap = data ? extractByDeptoRule(data, 'veiculos_usados', regras) : new Map();
        mergeAssistenciaMedica(valMap);
        mergeSalariosOrdenados(valMap);
        mergeComissoesEmpregados(valMap);
        mergeHorasExtras(valMap);
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsAudi(year, month, newObs);
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center py-16">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 font-medium">Nenhum dado importado para {periodoLabel}</p>
          <p className="text-slate-400 text-sm">Importe os dados na aba "Importar Dados" para visualizar.</p>
        </div>
      </div>
    );
  }
  ...
  if (totalContas === 0) {
    return (
      <div className="flex-1 flex items-center justify-center py-16">
        <div className="text-center space-y-2">
          <FileText className="w-10 h-10 text-slate-300 mx-auto" />
          <p className="text-slate-500 font-medium">Nenhuma conta classificada para {periodoLabel}</p>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          ...
          <p className="text-xs text-slate-400 mt-0.5">ADM Audi — Sorana Audi + Sorana Lapa</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Total de Despesas</p>
          <p className="text-lg font-bold text-slate-800 mt-1 tabular-nums">R$&nbsp;{fmtBRL(grandTotal)}</p>
          <p className="text-xs text-slate-400 mt-0.5">
            {activeGroups.length} grupos · {totalContas} conta{totalContas !== 1 ? 's' : ''}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Maior Grupo</p>
          {topGroup.tipo ? (
            <>
              <p className="text-sm font-bold mt-1 leading-tight" style={{ color: GRUPO_CONFIG[topGroup.tipo].color }}>
                {TIPO_LABELS[topGroup.tipo]}
              </p>
              <p className="text-xs text-slate-400 mt-0.5 tabular-nums">R$&nbsp;{fmtBRL(topGroup.total)}</p>
            </>
          ) : null}
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-xl px-5 py-3.5 flex items-center justify-between shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {periodoLabel} · ADM Audi (Sorana Audi + Sorana Lapa) · {totalContas} conta{totalContas !== 1 ? 's' : ''} em {activeGroups.length} grupo{activeGroups.length !== 1 ? 's' : ''}
          </p>
        </div>
