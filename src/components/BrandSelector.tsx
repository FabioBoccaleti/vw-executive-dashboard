...
const DEMONSTRATIVO_BRANDS: Brand[] = ['vw', 'audi', 'consolidado', 'resumo_dre', 'analise_projecoes', 'despesas_administracao', 'despesas_depto_audi', 'despesas_depto_vw', 'ieo', 'base_gerencial', 'comparativo_rede_vw', 'vw_outros', 'audi_outros'];
...
export function BrandSelector({ onSelectBrand, currentBrand, onAdminClick, onLogout }: BrandSelectorProps) {
  ...
  const showControlePagamentosEvora = canAccessModule('controle_pagamentos_evora');
  ...
  const allowedDirectBrands = DIRECT_BRANDS.filter(b => {
    ...
    if (b === 'controle_pagamentos_evora')    return showControlePagamentosEvora;
    return false;
  });
  ...
  const getBrandIcon = (brand: Brand, size = 'w-8 h-8') => {
    ...
    if (brand === 'base_gerencial')                  return <Layers className={size} />;
    if (brand === 'comparativo_rede_vw')             return <Layers className={size} />;
          ...
          {showDemonstrativo && (
            ...
            >
              ...
              {demonstrativoExpanded && (
                <div className="px-3 pb-4 grid grid-cols-6 gap-2 animate-in fade-in slide-in-from-top-2 duration-300 border-t border-slate-100 pt-3">
