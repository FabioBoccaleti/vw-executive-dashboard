...
const KEY = 'peliculas_cadastro_catalogo';

export async function loadPeliculasCatalogo(): Promise<CatalogoVeiculos> {
  ...
  try {
    ...
    if (audiMarca) {
      ...
      await savePeliculasCatalogo(seeded);
      return seeded;
    }
...
export async function savePeliculasCatalogo(catalogo: CatalogoVeiculos): Promise<boolean> {
