...
import {
  loadPeliculasAliquotas, savePeliculasAliquotas, type AliquotaImposto,
...
export function PeliculasAliquotasSection() {
  ...
  useEffect(() => {
    loadPeliculasAliquotas().then(d => { setItems(d); setLoading(false); });
  }, []);

  const persist = async (updated: AliquotaImposto[]) => {
    setSaving(true);
    try {
      const ok = await savePeliculasAliquotas(updated);
