...
import { loadPeliculasDsr, savePeliculasDsr, type DsrConfig } from '../cadastrosStorage';
...
export function PeliculasDsrSection() {
  ...
  useEffect(() => {
    loadPeliculasDsr().then(d => { setItems(d); setLoading(false); });
  }, []);

  const persist = async (updated: DsrConfig[]) => {
    setSaving(true);
    try {
      const ok = await savePeliculasDsr(updated);
