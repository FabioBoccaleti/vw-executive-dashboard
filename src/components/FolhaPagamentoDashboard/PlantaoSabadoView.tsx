...
import { Plus, Pencil, Trash2, X, Check, Printer, PenLine, ShieldCheck, DollarSign, LockOpen, ClipboardList, FileText, CalendarPlus } from 'lucide-react';
...
import {
  ...
  loadDiasExtra, saveDiasExtra,
  loadSelecao, saveSelecao,
  loadTitulos, saveTitulos, tituloEfetivo,
  loadLancamentos, saveLancamentos,
  sabadosDoMes, fmtDiaBR, isSabado,
  DEPTO_LABELS, DEPTOS,
  type PlantaoColaborador, type PlantaoDepto,
  type DiasExtraMap, type SelecaoMap, type TituloMap,
  type LancamentosMap, type PlantaoLancamento, type PlantaoLinhaSnapshot,
...
export function PlantaoSabadoView({ onCompetencia }: { onCompetencia?: (year: number, month: number) => void }) {
  ...
  const [colaboradores, setColaboradores] = useState<PlantaoColaborador[]>([]);
  const [diasExtra, setDiasExtra] = useState<DiasExtraMap>({});
  const [selecao, setSelecao] = useState<SelecaoMap>({});
  const [titulos, setTitulos] = useState<TituloMap>({});
  const [lancamentos, setLancamentos] = useState<LancamentosMap>({});
  const [loading, setLoading] = useState(true);

  const [tituloDraft, setTituloDraft] = useState('');
  const [dialog, setDialog] = useState<{ edit?: PlantaoColaborador } | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      loadColaboradores(), loadDiasExtra(), loadSelecao(), loadTitulos(), loadLancamentos(),
    ]).then(([c, de, se, ti, la]) => {
      setColaboradores(c);
      setDiasExtra(de);
      setSelecao(se);
  ...
  const diasDisponiveis = useMemo(() => {
    const set = new Set<string>([
      ...sabadosDoMes(filterYear, filterMonth),
      ...sabadosDoMes(prev.y, prev.m),
      ...(diasExtra[pk] ?? []),
    ]);
    return [...set].sort();
  }, [filterYear, filterMonth, prev.y, prev.m, diasExtra, pk]);
  ...
  }, [colaboradores, selForPk]);

  // Ao pagar, usa o snapshot congelado
  ...
  // ── Persistência ────────────────────────────────────────────────────────────
  function persistColaboradores(next: PlantaoColaborador[]) {
    setColaboradores(next);
    saveColaboradores(next);
  }
  function persistSelecao(next: SelecaoMap) {
    setSelecao(next);
    saveSelecao(next);
  }
  function persistDiasExtra(next: DiasExtraMap) {
    setDiasExtra(next);
    saveDiasExtra(next);
  }
  ...
  function handleSaveColaborador(data: Omit<PlantaoColaborador, 'id' | 'ativo' | 'ordem'>, editId?: string) {
    if (editId) {
  ...
  function addDiaAvulso() {
    if (pago) return;
    const d = novoDia;
    if (!d) return;
    if (diasDisponiveis.includes(d)) { toast.error('Esse dia já está na lista.'); setNovoDia(''); return; }
    persistDiasExtra({ ...diasExtra, [pk]: [...(diasExtra[pk] ?? []), d] });
    setNovoDia('');
    toast.success(`Dia ${fmtDiaBR(d)} adicionado.`);
  }
  ...
  async function marcarPago() {
    setSavingPago(true);
    try {
      const snapshot: PlantaoLinhaSnapshot[] = linhasLive.map(l => ({ ...l }));
      const upd: PlantaoLancamento = {
        ...lancamento,
        pago: true,
  ...
  async function confirmarAssinatura() {
    ...
    const upd: PlantaoLancamento = { ...existing, assinaturas: { ...(existing.assinaturas ?? {}), [assinaDialog.campo]: assinatura } };
    await persistLancamentos({ ...lancamentos, [pk]: upd });
  ...
  function handlePrint() {
    ...
    const rowsHtml = linhas.map(l => {
      totalGeral += l.total; totaisDep[l.departamento] += l.total;
    ...
    <div className="flex-1 flex flex-col overflow-hidden" style={{ minHeight: 0 }}>
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <input value={tituloDraft} onChange={e => setTituloDraft(e.target.value)} disabled={pago} placeholder="ex: Prêmio Plantão de Sábado" className="border border-slate-200 rounded px-2.5 py-1.5 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-teal-400 disabled:bg-slate-50" />
...
function PremioTab({
  ...
  competencia: string;
  novoDia: string;
  setNovoDia: (s: string) => void;
  onAddDia: () => void;
  onToggleDia: (colabId: string, dia: string) => void;
  onNew: () => void;
  onEdit: (c: PlantaoColaborador) => void;
  onDelete: (id: string) => void;
}) {
  const [pwd, setPwd] = useState<{ kind: 'edit' | 'delete'; colab: PlantaoColaborador; senha: string; erro: string | null } | null>(null);
    ...
    <div className="max-w-5xl mx-auto p-6 flex flex-col gap-4">
      {pago && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-2.5 text-sm text-emerald-700">
          O demonstrativo de <strong>{competencia}</strong> está <strong>pago</strong> — a seleção de dias está bloqueada. Reabra o demonstrativo para editar.
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Marque os dias trabalhados em {competencia}.</p>
        </div>
      ...
      <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-4 py-2.5">
        ...
        <span className="text-xs text-slate-500">Adicionar dia avulso (ex: feriado):</span>
        <input type="date" value={novoDia} onChange={e => setNovoDia(e.target.value)} disabled={pago} className="border border-slate-200 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 disabled:bg-slate-50" />
        <button onClick={onAddDia} disabled={pago || !novoDia} className="text-xs bg-slate-700 hover:bg-slate-800 disabled:opacity-40 text-white rounded px-3 py-1.5 font-semibold">Adicionar</button>
      </div>
      ...
      {ativos.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {ativos.map(c => {
            const dias = selForPk[c.id] ?? [];
            const total = c.valorPorSabado * dias.length;
            return (
              <div key={c.id} className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[10px] text-slate-400">R$ {fmtBRL(c.valorPorSabado)} / dia</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-slate-400">{dias.length} {dias.length === 1 ? 'dia' : 'dias'}</p>
                    </div>
                ...
                <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-100">
                  {diasDisponiveis.length === 0 ? (
                    <span className="text-xs text-slate-400 italic">Nenhum dia disponível.</span>
...
function DemonstrativoTab({
  ...
  competencia: string;
  linhas: LinhaDemo[];
  deptosPresentes: PlantaoDepto[];
  showTotal: boolean;
  pago: boolean;
  lancamento: PlantaoLancamento | undefined;
  savingPago: boolean;
  onMarcarPago: () => void;
  onReabrir: () => void;
  ...
  let totalGeral = 0;
  let totalDias = 0;
  linhas.forEach(l => { totalGeral += l.total; totaisDep[l.departamento] += l.total; totalDias += l.dias.length; });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 flex flex-col gap-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-4">
          {linhas.length === 0 ? (
            <p className="text-center text-slate-400 text-sm py-8">Nenhum colaborador com dias selecionados em {competencia}.</p>
          ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 py-8 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 my-auto">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Valor por sábado</label>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select value={departamento} onChange={e => setDepartamento(e.target.value as PlantaoDepto)} className="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
