...
export function DepartamentoTab({ marca, departamento, year, mes }: Props) {
      ...
      try {
        const result =
          departamento === 'consolidado'
            ? await processConsolidadoData(marca, year, mes)
            : await processDepartamentoData(marca, departamento, year, mes);
        setData(result);
      } catch (error) {
