...
import {
  ...
  type BaseGerencialIndicadorConfig,
  type BaseGerencialCenario,
  type NumeradorTipo,
...
const DEFAULT_PERIODOS: Periodo[] = [
  { year: CURRENT_YEAR - 1, mes: CURRENT_MONTH },
  { year: CURRENT_YEAR, mes: CURRENT_MONTH === 1 ? 12 : CURRENT_MONTH - 1 },
  { year: CURRENT_YEAR, mes: CURRENT_MONTH },
];
...
function fmtPeriodo(p: Periodo) {
  return `${MESES_ABREV[p.mes - 1]}/${p.year}`;
}

function fmtVal(n: number | null, formato: BaseGerencialIndicadorConfig['formato']): string {
  if (n === null) return '—';
...
function fmtMeta(meta: number, formato: BaseGerencialIndicadorConfig['formato']): string {
...
const SENHA_BASE_GERENCIAL = '1985';
...
function CenarioSection({
  ...
  const impactRows = periodos.map(p => {
    const data = deptoDataMap[`${p.year}:M${String(p.mes).padStart(2, '0')}`];
    if (!data) return null;
    return {
            ...
            <tbody className="divide-y divide-slate-50">
              {cenario.indicadores.map(ind => {
                const vals = periodos.map(p => {
                  const key = `${p.year}:M${String(p.mes).padStart(2, '0')}`;
                  const data = deptoDataMap[key];
                  if (!data) return null;
                  const dadosOp = allDadosOp[dadosOpKey(p.year, p.mes, marca, cenario.departamento)] ?? {};
...
interface MarcaAnaliseProps {
  ...
  onEdit: (c: BaseGerencialCenario) => void;
  onDelete: (id: string) => void;
  onMove: (id: string, dir: 'up' | 'down') => void;
  onDuplicate: (c: BaseGerencialCenario) => void;
...
function MarcaAnalise({
  ...
  useEffect(() => {
    ...
    Promise.all(
      periodos.map(async p => {
        ...
        const data =
          depto === 'consolidado'
            ? await processConsolidadoData(marca, p.year, p.mes)
            : await processDepartamentoData(marca, depto, p.year, p.mes);
  ...
  function addPeriodo() {
    ...
    const next: Periodo = last.mes === 12 ? { year: last.year + 1, mes: 1 } : { year: last.year, mes: last.mes + 1 };
    onPeriodosChange([...periodos, next]);
  }
      ...
      <div className="flex items-center gap-2 flex-wrap bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
        ...
        {periodos.map((p, idx) => (
          ...
          >
            ...
            <select
              value={p.mes}
              onChange={e => updatePeriodo(idx, 'mes', Number(e.target.value))}
              className="text-xs font-semibold text-slate-700 bg-transparent border-none outline-none cursor-pointer"
            >
...
function ConfiguracoesCenarios({ cenarios, showDespFin, onShowDespFinChange, onEdit, onDelete, onNew }: ConfigProps) {
  ...
  const [pendingItem, setPendingItem] = useState<{ action: 'edit' | 'delete'; cenario: BaseGerencialCenario } | null>(null);
  const [dadosOpYear, setDadosOpYear] = useState(CURRENT_YEAR);
  const [dadosOpMes, setDadosOpMes] = useState(CURRENT_MONTH);
  ...
  async function handleDadosOpChange(
    ...
    const key = dadosOpKey(dadosOpYear, dadosOpMes, marca, dept);
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {MESES_ABREV.map((label, i) => (
              <button
                key={i}
                onClick={() => setDadosOpMes(i + 1)}
                className={`px-3 py-1.5 text-xs font-semibold rounded transition-colors ${
                  dadosOpMes === i + 1 ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:bg-slate-100'
                }`}
              >
                {label}
              </button>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {DEPTOS_OPTS.map(({ id, label }) => {
                  const vwVal = allDadosOp[dadosOpKey(dadosOpYear, dadosOpMes, 'vw', id)] ?? {};
                  const audiVal = allDadosOp[dadosOpKey(dadosOpYear, dadosOpMes, 'audi', id)] ?? {};
                  return (
...
interface EditorProps {
  initial?: BaseGerencialCenario;
  defaultDepto?: DeptoFiltro;
  onSave: (c: BaseGerencialCenario) => void;
  onClose: () => void;
}
...
function CenarioEditorDialog({ initial, defaultDepto, onSave, onClose }: EditorProps) {
  ...
  const [indicadores, setIndicadores] = useState<BaseGerencialIndicadorConfig[]>(
    initial?.indicadores ?? [],
  );
  ...
  function updateIndicador(id: string, patch: Partial<BaseGerencialIndicadorConfig>) {
                ...
                <div key={ind.id} className="border border-slate-200 rounded-xl p-4 space-y-3 bg-slate-50">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <select
                        value={ind.formato}
                        onChange={e => updateIndicador(ind.id, { formato: e.target.value as BaseGerencialIndicadorConfig['formato'] })}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-1 focus:ring-emerald-400"
                      >
...
export function AnaliseCenariosTab() {
  ...
  const [cenarios, setCenarios] = useState<BaseGerencialCenario[]>([]);
  const [allDadosOp, setAllDadosOp] = useState<Record<string, DadosOperacionais>>({});
  ...
  const [showDespFin, setShowDespFin] = useState<boolean>(
    () => localStorage.getItem('basege:desp_fin_visivel') !== 'false',
  );
  const [editing, setEditing] = useState<{
    cenario?: BaseGerencialCenario;
    defaultDepto?: DeptoFiltro;
  } | null>(null);
  const [duplicating, setDuplicating] = useState<BaseGerencialCenario | null>(null);

  function handleShowDespFinChange(v: boolean) {
    setShowDespFin(v);
    localStorage.setItem('basege:desp_fin_visivel', String(v));
  }
  ...
  async function handleSave(cenario: BaseGerencialCenario) {
  ...
  async function handleDuplicate(cenario: BaseGerencialCenario, deptos: Array<DeptoClassificacao | 'consolidado'>) {
    const novosCenarios: BaseGerencialCenario[] = deptos.map(depto => ({
      ...cenario,
      id: genId(),
      departamento: depto,
