...
function periodoKey(p: Periodo) {
  return `${p.year}:M${String(p.mes).padStart(2, '0')}`;
}
...
export function ComparativoTab() {
  ...
  function addPeriodo() {
    ...
    const next: Periodo =
      last.mes === 12
        ? { year: last.year + 1, mes: 1 }
        : { year: last.year, mes: last.mes + 1 };
  ...
  function updatePeriodo(idx: number, field: 'year' | 'mes', value: number) {
        ...
        {periodos.map((p, idx) => (
          ...
          >
            ...
            <select
              ...
              className="text-xs font-semibold text-slate-700 bg-transparent border-none outline-none cursor-pointer"
            >
              {MESES_ABREV.map((label, idx2) => (
                <option key={idx2} value={idx2 + 1}>{label}</option>
              ))}
            </select>
            {periodos.length > 1 && (
              <button
