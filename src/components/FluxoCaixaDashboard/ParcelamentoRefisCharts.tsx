/**
 * Gráficos de Evolução — Parcelamento Refis
 * Exibe 1 gráfico de barras com o total (CP + LP) mês a mês.
 * Contas: 2.1.2.02.07.020 (Curto Prazo) + 2.2.1.08.01.020 (Longo Prazo).
...
const CONTA_CP = '2.1.2.02.07.020';
const CONTA_LP = '2.2.1.08.01.020';

const MONTH_LABELS = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];

// ─── Parser simplificado (extrai as 2 contas Refis) ──────────────────────────
function extractRefisValues(rawText: string): { cp: number; lp: number } {
  let cp = 0;
  let lp = 0;
...
export function ParcelamentoRefisCharts({ selectedYear, selectedMonth, onClose }: Props) {
  ...
  const loadYearData = useCallback(async (year: number, upToMonth: number) => {
    ...
    for (const month of months) {
      ...
      if (!raw) continue;
      const { cp, lp } = extractRefisValues(raw);
      totals[month] = cp + lp;
    }
  ...
  const buildChartData = () => {
    ...
    return months.map((m, idx) => {
      const curVal = totalData[m] || 0;
      const prevCurVal = idx > 0 ? (totalData[months[idx - 1]] || 0) : null;
      ...
      if (cmpKey) {
        const cmpVal = compareTotalData[m] || 0;
        const prevCmpVal = idx > 0 ? (compareTotalData[months[idx - 1]] || 0) : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold text-foreground">
            Evolução Mensal — Parcelamento Refis
          </h3>
          <p className="text-sm text-muted-foreground mt-0.5">
            {selectedYear} • Jan{upToMonth > 1 ? ` – ${MONTH_LABELS[upToMonth - 1]}` : ''} • Total CP + LP
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <Card>
        <CardContent className="pt-5 pb-4">
          <div className="mb-3">
            <h4 className="text-base font-bold text-foreground">Total Parcelamento Refis</h4>
            <p className="text-xs text-muted-foreground">CP (2.1.2.02.07.020) + LP (2.2.1.08.01.020)</p>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={buildChartData()} barGap={compareYear ? 2 : 0}>
