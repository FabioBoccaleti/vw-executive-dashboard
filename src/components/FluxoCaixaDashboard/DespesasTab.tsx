                  ...
                  <div className="flex items-start gap-4">
                    {showYTD && (
                      <div className="text-right">
                        <div className="text-xs text-amber-600 dark:text-amber-400 font-medium mb-0.5">{ytdLabel}</div>
                        <div className="text-sm font-bold font-mono text-amber-600 dark:text-amber-400 whitespace-nowrap">
                    ...
                    <table className="w-full">
                      {showYTD && (
                        <thead>
                          <tr className="border-b border-border/30">
                            <th className="py-1.5 pl-4 text-left text-xs text-muted-foreground font-medium w-full"></th>
                            <th className="py-1.5 px-2 text-right text-xs text-muted-foreground font-medium whitespace-nowrap">%</th>
                            <th className="py-1.5 pl-2 pr-2 text-right text-xs text-muted-foreground font-medium whitespace-nowrap">Mês</th>
                            <th className="py-1.5 pl-2 pr-4 text-right text-xs text-amber-600 dark:text-amber-400 font-semibold whitespace-nowrap border-l border-border/30">{ytdLabel}</th>
                          </tr>
                        </thead>
                      )}
                      <tbody>
                        {grupo.itens.map(([tipo, valor], i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className={`py-2 pl-2 ${showYTD ? 'pr-2' : 'pr-4'} text-right text-sm font-mono font-semibold text-red-600 dark:text-red-400 whitespace-nowrap`}>
                                {fmtBRL(valor)}
                              </td>
                              {showYTD && (
                                <td className="py-2 pl-2 pr-4 text-right text-sm font-mono font-semibold text-amber-600 dark:text-amber-400 whitespace-nowrap border-l border-border/30">
                                  {fmtBRL(resumoMapYTD[tipo] ?? 0)}
                                </td>
                              )}
                            </tr>
                          );
                      ...
                      <tfoot>
                        <tr className={`border-t border-border/50 ${grupo.headerBg}`}>
                          ...
                          <td></td>
                          <td className={`py-2 ${showYTD ? 'pr-2' : 'pr-4'} text-right text-sm font-mono font-bold ${grupo.totalColor} whitespace-nowrap`}>
                            {fmtBRL(grupo.total)}
                          </td>
                          {showYTD && (
                            <td className={`py-2 pl-2 pr-4 text-right text-sm font-mono font-bold text-amber-600 dark:text-amber-400 whitespace-nowrap border-l border-border/30`}>
                              {fmtBRL(grupo.totalYTD)}
                            </td>
                          )}
                        </tr>
