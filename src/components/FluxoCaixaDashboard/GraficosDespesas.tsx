/**
 * Página de Gráficos de Despesas — exibe um gráfico de barras lado a lado
 * por subcategoria, para cada grupo selecionado no Comparativo.
 */
...
export interface GrupoDadosRow {
  tipo: string;
  valorA: number;
  valorB: number;
}
...
// ─── Helpers de período (copiados de ComparativoDespesas para ser standalone) ─
...
function parseGrupo5(rawText: string): Record<string, { valDeb: number; valCred: number }> {
  ...
  for (const line of rawText.split('\n')) {
    ...
    result[conta] = { valDeb: parse(parts[4]), valCred: parse(parts[5]) };
  }
  return result;
}

function grupo5Leaves(accounts: Record<string, { valDeb: number; valCred: number }>) {
  const allKeys = Object.keys(accounts);
  return allKeys
    .filter((k) => !allKeys.some((o) => o !== k && o.startsWith(k + '.')))
    .sort()
...
interface GraficosDespesasProps {
  /** Dados iniciais já carregados no Comparativo */
  initialGrupos: GrupoResult[];
...
export function GraficosDespesas({
  ...
  const [grupos, setGrupos] = useState<GrupoResult[]>(initialGrupos);
  const [tipos] = useState(initialTipos);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // IDs de grupo selecionados (controláveis pelo usuário)
  const [selectedGrupo1Id, setSelectedGrupo1Id] = useState<string>(initialGrupos[0]?.grupoId ?? '');
  ...
  function buildRows(
    ...
    for (const conta of allContas) {
      ...
      if (tipoLabel && classificarTipo(tipoLabel) === grupoIdx) tiposSet.add(tipoLabel);
    }
    return Array.from(tiposSet).map((tipoLabel) => {
  ...
  // Formata valor compacto para eixo Y
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
            <span>📈</span> Gráficos de Despesas
          </h2>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={cn(
                ...
                loading ? 'bg-muted text-muted-foreground cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 text-white'
              )}
            >
              {loading ? (
      ...
      {grupos.map(({ grupoId, rows }) => {
        const grupoConfig = GRUPOS_CONFIG.find((g) => g.id === grupoId);
        ...
        return (
          <div key={grupoId} className="space-y-4">
            ...
            {/* Grid de gráficos — 1 por subcategoria */}
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {rows.map(({ tipo, valorA, valorB }) => {
                const chartData = [
                ...
                return (
                  <Card key={tipo} className="shadow-sm overflow-hidden">
                    ...
                    <CardContent className="px-2 pb-4">
                      ...
                      {/* Mini resumo embaixo do gráfico */}
