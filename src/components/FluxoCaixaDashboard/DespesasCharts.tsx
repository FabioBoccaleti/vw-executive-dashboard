/**
 * Gráficos de Evolução — Despesas
 * Gráficos individuais por conta (grupo 5.*), separados pelos 7 grupos
 * definidos em GRUPOS_CONFIG (Pessoal, Veículos, Imóveis, Terceiros,
 * Financeiras, Marketing, Outras) + Total Geral.
 * Classificação usa regex sobre a descrição, igual à DespesasTab.
 */
...
function extractExpenseAccounts(rawText: string): Record<string, AccData> {
...
/** Descobre contas folha (sem filhos) */
function discoverLeaves(accsMap: Record<string, AccData>): string[] {
  const allIds = Object.keys(accsMap);
  return allIds
    .filter(k => !allIds.some(other => other !== k && other.startsWith(k + '.')))
    .sort();
}
...
export function DespesasCharts({ selectedYear, selectedMonth, onClose }: Props) {
  ...
  const loadYearData = useCallback(async (year: number, upToMonth: number) => {
    ...
    for (const month of months) {
      ...
      if (!raw) continue;
      const accs = extractExpenseAccounts(raw);
    ...
    // Discover leaf accounts across all months
    const allMerged: Record<string, AccData> = {};
    for (const accs of Object.values(monthAccs)) {
      for (const [k, v] of Object.entries(accs)) {
        if (!allMerged[k]) allMerged[k] = v;
      }
    }
    const leaves = discoverLeaves(allMerged);

    // Build chart items, classify each account
    const chartItems: ChartItem[] = [];
    const totals: Record<number, number> = {};

    for (const id of leaves) {
      const desc = descMap[id] || id;
      const groupIdx = classificarTipo(desc);

      const item: ChartItem = {
        key: id,
        label: toTitleCase(desc),
        groupIdx,
        values: {},
      };
  ...
  // Group active charts by grupo
  const groupedCharts = GRUPOS_CONFIG.map((g, idx) => ({
    label: `${g.icon} ${g.label}`,
    items: activeCharts.filter(c => c.groupIdx === idx),
        ...
        <div key={section.label}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            {section.items.map((chart) => {
              ...
              const compareVals = compareCharts.find(c => c.key === chart.key)?.values || {};
              ...
              return (
                <Card key={chart.key}>
                  <CardContent className="pt-5 pb-4">
                    <div className="mb-3">
                      <span className="text-xs font-mono text-muted-foreground">{chart.key}</span>
                      <div className="flex items-center justify-between">
      ...
      <Card>
        <CardContent className="pt-5 pb-4">
          <div className="mb-3">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-base font-bold text-foreground">Total Geral — Despesas</h4>
