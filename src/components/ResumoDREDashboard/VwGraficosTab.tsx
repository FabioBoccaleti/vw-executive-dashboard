...
function sumField(row: DreVwRow, field: keyof DreVwDept, deptFilter: DeptFilter): number {
...
function accumulate(rows: DreVwRow[], upToIdx: number): DreVwRow {
...
const WF_STEPS: { label: string; field: keyof DreVwDept }[] = [
...
function WaterfallPanel({
  ...
  row: DreVwRow; title: string; subtitle: string; deptFilter: DeptFilter;
}) {
  const data = WF_STEPS.map(s => ({ label: s.label, value: sumField(row, s.field, deptFilter) }));
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex-1">
      <div className="px-4 py-2.5" style={{ backgroundColor: VW_COLOR }}>
      ...
      <div className="p-3">
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 24 }}>
            ...
            <Bar dataKey="value" radius={[4, 4, 0, 0]} name="Valor">
              {data.map((d, i) => (
                <Cell key={i} fill={d.value >= 0 ? (i === data.length - 1 ? VW_COLOR : '#22c55e') : '#ef4444'} />
              ))}
            </Bar>
        ...
        <div className="flex justify-center gap-4 text-[0.6rem] text-slate-400 mt-1">
          ...
          <span className="flex items-center gap-1"><span className="w-2.5 h-2 rounded-sm inline-block" style={{ backgroundColor: VW_COLOR }} />Lucro Líq.</span>
        </div>
      </div>
    </div>
  );
...
function DonutPanel({
  ...
  row: DreVwRow; field: keyof DreVwDept; title: string; subtitle: string; deptFilter: DeptFilter;
}) {
  const data = DEPTS
...
export function VwGraficosTab({ year, month }: Props) {
  ...
  const mesRow   = allMonthRows[selIdx] ?? createEmptyDreVwRow(year, selIdx + 1);
  ...
  const sumDesp     = (row: DreVwRow) => deptList.reduce((s, d) =>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50">
      <div id="vw-graficos-print-area" className="max-w-[1440px] mx-auto p-4 flex flex-col gap-4">

        {/* ── Header ──────────────────────────────────────────────────────── */}
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <h2 className="text-lg font-bold text-slate-800">VW — Análise Gráfica</h2>
            <p className="text-sm text-slate-500">{periodoFull} · Acumulado: {accumLabel}</p>
          </div>
          <button
            onClick={() => {
              const area = document.getElementById('vw-graficos-print-area');
          ...
          {(['consolidado', ...DEPTS.map(d => d.key)] as DeptFilter[]).map(key => {
            ...
            return (
              <button key={key} onClick={() => setDeptFilter(key)}
                ...
                style={isActive ? { backgroundColor: dept?.color ?? VW_COLOR } : {}}
              >
            ...
            <div className="p-2">
              <ResponsiveContainer width="100%" height={230}>
                <PieChart>
                  ...
                  <Tooltip content={({ active, payload }) => {
                    if (!active || !payload?.length) return null;
                    const p = payload[0]; const tot = despMes.reduce((s, d) => s + d.value, 0);
            ...
            <div className="p-2">
              <ResponsiveContainer width="100%" height={230}>
                <PieChart>
                  ...
                  <Tooltip content={({ active, payload }) => {
                    if (!active || !payload?.length) return null;
                    const p = payload[0]; const tot = despAcum.reduce((s, d) => s + d.value, 0);
                    const pct = tot > 0 ? ((p.value as number) / tot * 100).toFixed(1) : '0.0';
                    return <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-2.5 text-xs"><p className="font-bold text-slate-700 mb-1">{p.name}</p><p className="text-slate-600">R$ {fmtBRL(p.value as number)}</p><p className="font-semibold" style={{ color: p.payload.color }}>{pct}% do total</p></div>;
                  }} />
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 10 }} />
            ...
            <div className="p-4">
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={evolucao} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Line type="monotone" dataKey="lucro"   name="Lucro Líquido"       stroke={VW_COLOR} strokeWidth={2.5} dot={{ r: 4, fill: VW_COLOR }} />
                  <ReferenceLine y={0} stroke="#cbd5e1" strokeWidth={1.5} />
                </LineChart>
              </ResponsiveContainer>
            </div>
