...
const DEPTS_LIST: DeptKey[] = ['novos', 'usados', 'pecas', 'oficina', 'funilaria', 'adm'];
...
function consolidateDepts(row: DreAudiRow): DreAudiDept {
...
async function loadPeriodData(
  ...
  await Promise.all(
    DEPTS_LIST.map(async (deptKey) => {
      try { dreByDept[deptKey] = await loadDREDataAsync(yr, DEPT_KEY_TO_DEPT[deptKey], 'audi'); }
  ...
  const processedRows: DreAudiRow[] = monthRows.map((kvRow, i) => {
  ...
  const summedRow = createEmptyDreAudiRow(sel.year, 0);
...
interface AudiComparativoTabProps {
  year: number;
  month: number;
}

export function AudiComparativoTab({ year, month }: AudiComparativoTabProps) {
  ...
  const [loadedRows, setLoadedRows] = useState<(DreAudiRow | null)[]>([]);
            ...
            <div className="flex gap-1">
              {(['mensal', 'bimestral', 'trimestral', 'semestral', 'anual'] as Granularidade[]).map(g => (
                <button
                  ...
                  style={
                    granularidade === g
                      ? { backgroundColor: AUDI_COLOR, color: 'white' }
              ...
              {periodos.map((p, idx) => (
                ...
                >
                  <span
                    ...
                    style={{ backgroundColor: AUDI_COLOR }}
                  >
                    #{idx + 1}
                  </span>
        ...
        createPortal(
          <div style={{ fontFamily: 'Inter, sans-serif', colorScheme: 'only light' as any }}>
            <style>{`
              ...
              .audi-comp-header { background-color: ${AUDI_COLOR} !important; color: white !important; }
            ...
            <div className="print-page">
              <div className="audi-comp-header" style={{ backgroundColor: AUDI_COLOR, color: 'white', padding: '3px 8px', marginBottom: '3px', fontWeight: 700, fontSize: '9pt' } as React.CSSProperties}>
                Resumo Geral
              </div>
              <AudiComparativoTable
                periodos={periodos}
...
function AudiComparativoTable({
  ...
  loadedRows: (DreAudiRow | null)[];
  activeSection: 'resumo' | DeptKey;
  granularidade: Granularidade;
}) {
  const totalCols = 1 + 1 + (periodos.length - 1) * 3;

  const consolidados: (DreAudiDept | null)[] = loadedRows.map(row => {
    if (!row) return null;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="text-white px-6 py-3" style={{ backgroundColor: AUDI_COLOR }}>
        <h2 className="font-bold text-base">AUDI LAPA/PINHEIROS — Comparativo</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          {/* ── Cabeçalho ──────────────────────────────────────────────── */}
          <thead>
            <tr className="bg-slate-200" style={{ borderBottom: `2px solid ${AUDI_COLOR}` }}>
          ...
          <tbody>
            {DRE_LINES.map((line, idx) => {
              ...
              const rowStyle = line.isTotal ? { backgroundColor: AUDI_COLOR } : undefined;
                    ...
                    return (
                      <Fragment key={p.id}>
                        <td
                          ...
                          style={line.isTotal ? { backgroundColor: AUDI_COLOR_DRK } : undefined}
                        >
                          {display}
                        </td>
                        {pIdx > 0 && (
