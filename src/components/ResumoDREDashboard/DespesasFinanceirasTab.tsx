        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-4">
          ...
          <button
            onClick={() => {
              ...
              const monthHeaders = monthNums.map(m => MONTHS_SHORT[m - 1]);
              const header = ['Tipo de Despesa', ...monthHeaders, 'Total YTD', `vs ${prevYear}`, 'Var R$', 'Var %'];
              const dataRows = typeRows.map(r => [
                r.typeLabel,
                ...r.monthVals,
                r.ytdCurr,
                r.ytdPrev,
              ...
              ws['!cols'] = [{ wch: 32 }, ...monthNums.map(() => ({ wch: 14 })), { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 10 }];
              const wb = XLSX.utils.book_new();
              const period = upToMonth === 12 ? `Jan-Dez${year}` : `Jan-${MONTHS_SHORT[upToMonth - 1]}${year}`;
              XLSX.utils.book_append_sheet(wb, ws, `Desp Fin ${brandLabel} ${year}`.substring(0, 31));
              XLSX.writeFile(wb, `Despesas_Financeiras_${brandLabel.replace(' ', '_')}_${period}.xlsx`);
            }}
