...
/** Extrai Volume de Vendas do resultado de loadDREDataAsync para um dado mês (0-based). */
function extractVolume(dreLines: any[] | null, monthIdx: number): number {
  if (!dreLines) return 0;
  for (const line of dreLines) {
    const desc = ((line.descricao as string) || (line.label as string) || '').toUpperCase().trim();
    if (desc === 'VOLUME DE VENDAS') {
...
/** Volume de Vendas do KV manual (DreVwRow/DreAudiRow) para um dept. */
function getKvVolume(row: DreVwRow | DreAudiRow | null, deptKey: string): number {
  if (!row) return 0;
  const dept = (row as Record<string, any>)[deptKey];
  if (!dept) return 0;
  return parseVal(dept.quant);
}
...
export function VolumeVendasEvolucaoTab({ year, month }: Props) {
  ...
  function getVwVol(mIdx: number, deptKey: string, usePrev: boolean): number {
    const kv = usePrev ? vwPrev[mIdx] : vwCurr[mIdx];
    const kvVal = getKvVolume(kv ?? null, deptKey);
    if (kvVal !== 0) return kvVal;
    const async_ = usePrev ? vwAsync[deptKey]?.prev : vwAsync[deptKey]?.curr;
    return extractVolume(async_ ?? null, mIdx);
  }

  function getAudiVol(mIdx: number, deptKey: string, usePrev: boolean): number {
  ...
  function getCurr(mIdx: number, deptKey: string): number {
    if (subBrand === 'vw')   return getVwVol(mIdx, deptKey, false);
    if (subBrand === 'audi') return getAudiVol(mIdx, deptKey, false);
    // consolidado — deptKey 'direta' só existe no VW
    return getVwVol(mIdx, deptKey, false) + (deptKey !== 'direta' ? getAudiVol(mIdx, deptKey, false) : 0);
  }

  function getPrev(mIdx: number, deptKey: string): number {
    if (subBrand === 'vw')   return getVwVol(mIdx, deptKey, true);
    if (subBrand === 'audi') return getAudiVol(mIdx, deptKey, true);
    return getVwVol(mIdx, deptKey, true) + (deptKey !== 'direta' ? getAudiVol(mIdx, deptKey, true) : 0);
  }
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center gap-2">
        ...
        <span className="text-sm text-slate-500">Carregando volumes...</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6 space-y-6">
      ...
      {/* ── Title ───────────────────────────────────────────────────────── */}
      <div>
        <h2 className="text-xl font-bold text-slate-800">Análise Evolutiva de Volume de Vendas</h2>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Volume YTD {year}</p>
          <p className="text-2xl font-bold" style={{ color: brandColor }}>{fmtInt(totalCurr)} un.</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Volume YTD {prevYear}</p>
          <p className="text-2xl font-bold text-slate-400">{fmtInt(totalPrev)} un.</p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Variação Unid.</p>
          <p className={`text-2xl font-bold ${totalVar >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            {totalVar >= 0 ? '+' : ''}{fmtInt(totalVar)} un.
          </p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
          <h3 className="text-sm font-bold text-slate-700">Volume Mensal — {year} vs {prevYear}</h3>
          ...
          <ResponsiveContainer width="100%" height={270}>
            <BarChart data={chartData} barCategoryGap="25%" barGap={3}>
              ...
              <Tooltip
                formatter={(value: any, name: any) => [`${fmtInt(Number(value))} un.`, name]}
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div className="bg-white p-3 rounded-lg shadow-lg border border-slate-200 text-xs">
                      ...
                      <p className="text-slate-600">{year}: <span className="font-semibold">{fmtInt(d.curr)} un.</span></p>
                      <p className="text-slate-400">{prevYear}: {fmtInt(d.prev)} un.</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-700">Volume de Vendas por Departamento</h3>
          <p className="text-xs text-slate-400 mt-0.5">Unidades — acumulado Jan a {MONTHS_SHORT[upToMonth - 1]}/{year} · comparação com {prevYear}</p>
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-3 py-3 text-right font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">Var Unid.</th>
