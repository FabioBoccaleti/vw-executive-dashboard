...
export function ConsolidadoAdmTab({ year, month }: Props) {
    ...
    const load = async () => {
      ...
      if (isYearMode) {
        ...
        for (const data of allMonths) {
          if (!data) continue;
          for (const [conta, val] of extractConsolidadoValues(data)) {
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsConsolidado(year, month, newObs);
  }
