...
export async function printDespesasAdm(year: number, month: number): Promise<void> {
  const [classif, ...allMonthDatas] = await Promise.all([
    loadClassificacoes(),
  ...
  for (const tab of TABS_CONFIG) {
    const extract = (data: NonNullable<typeof currentData>) => {
      const m = extractByCompaniesAndDepts(data, tab.companies, tab.depts);
...
<head>
  <meta charset="UTF-8"/>
  <title>Despesas na Administração — ${year}</title>
  <style>
