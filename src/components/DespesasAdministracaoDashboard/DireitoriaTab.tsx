...
export function DireitoriaTab({ year, month }: Props) {
    ...
    const load = async () => {
      ...
      if (isYearMode) {
        ...
        for (const data of allMonths) {
          if (!data) continue;
          for (const [conta, val] of extractDiretoriaValues(data)) {
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsDiretoria(year, month, newObs);
  }
