...
export function AdmVwTab({ year, month }: Props) {
    ...
    const load = async () => {
      ...
      if (isYearMode) {
        ...
        for (const data of allMonths) {
          if (!data) continue;
          for (const [conta, val] of extractSoranaVwValues(data)) {
  ...
  async function handleObsChange(newObs: Record<string, string>) {
    setObs(newObs);
    if (!isYearMode) await saveObsVw(year, month, newObs);
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
          ...
          <p className="text-xs text-slate-400 mt-0.5">ADM VW — Sorana Norte</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
      ...
      <div className="bg-slate-800 rounded-xl px-5 py-3.5 flex items-center justify-between shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {periodoLabel} · ADM VW (Sorana Norte) · {totalContas} conta{totalContas !== 1 ? 's' : ''} em {activeGroups.length} grupo{activeGroups.length !== 1 ? 's' : ''}
          </p>
        </div>
