...
export function VendasUsadoAnalise() {
  ...
  const [dailyMarca,     setDailyMarca]     = useState('Todas');
  const [dailyMetric,    setDailyMetric]    = useState<'receita' | 'qtd'>('receita');

  // Tabela Pivot
  const [pivotGrouping, setPivotGrouping] = useState<'marca' | 'modelo'>('marca');
  ...
  const dailyMarcas = useMemo(() => {
    const sourceRows = month === null ? yearRows : filteredRows;
    const s = new Set<string>();
    for (const r of sourceRows) s.add(normalizeMarca(r.modelo ?? ''));
    return ['Todas', ...[...s].sort()];
  ...
  const daysInMonthUsados = useMemo(() => {
    if (month === null) return [];
    return Array.from({ length: new Date(year, month, 0).getDate() }, (_, i) => i + 1);
  }, [year, month]);

  const dailyUsadosData = useMemo(() => {
    if (month === null) return [];
  ...
  }, [filteredRows, month, dailyVendedor, dailyMarca, daysInMonthUsados]);
  ...
  }, [yearRows, dailyVendedor, dailyMarca]);
  ...
  // ── Tabela Pivot Marca/Modelo × Meses ────────────────────────────────────
  ...
  const pivotData = useMemo(() => {
    ...
    const getKey = (r: VendasResultadoRow) =>
      pivotGrouping === 'marca' ? normalizeMarca(r.modelo ?? '') : (r.modelo?.trim() || '(sem modelo)');

    const map = new Map<string, Map<number, VendasResultadoRow[]>>();
    for (const r of yearRows) {
      if (r.transacao === 'U07') continue;
      const key = getKey(r);
      const mo  = getMo(r);
    ...
    for (const mo of pivotActiveMeses) {
      const mr = yearRows.filter(r => getMo(r) === mo && r.transacao !== 'U07');
      const a  = agg(mr, aliqBon, dsrCfg);
      totals[mo] = a
        ? { vol: a.netVol, lbPct: a.lbPct, margPct: a.marg, mediaDias: a.mediaDias, medianaDias: a.medianaDias }
        : { vol: 0, lbPct: 0, margPct: 0, mediaDias: 0, medianaDias: 0 };
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <SH right={
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <select
                  value={dailyMarca}
                  onChange={e => setDailyMarca(e.target.value)}
                  className="border border-slate-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-blue-400 max-w-[200px]"
                >
                  {dailyMarcas.map(f => <option key={f} value={f}>{f}</option>)}
                </select>
            ...
            {month === null ? (
              monthlyUsadosData.every(d => d.receita === 0 && d.qtd === 0) ? (
                <div className="h-24 flex items-center justify-center text-slate-300 text-xs">Sem dados no período</div>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={monthlyUsadosData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              dailyUsadosData.every(d => d.receita === 0 && d.qtd === 0) ? (
                <div className="h-24 flex items-center justify-center text-slate-300 text-xs">Sem dados no período</div>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={dailyUsadosData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            ...
            return (
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                <SH right={
                  ...
                  Comparativo Mensal por {pivotGrouping === 'marca' ? 'Marca' : 'Modelo'} — {year}
                </SH>
                    ...
                    <thead>
                      <tr className="border-b-2 border-slate-200">
                        <th className="sticky left-0 bg-white z-10 text-left py-2 pr-3 font-semibold text-slate-500 uppercase tracking-wide text-[10px] min-w-[120px]">
                          {pivotGrouping === 'marca' ? 'Marca' : 'Modelo'}
                        </th>
                        {meses.map(mo => (
                    ...
                    <tbody>
                      {rows.map((row, ri) => (
                        <tr key={row.name} className={`border-b border-slate-100 transition-colors hover:bg-slate-50 ${ri % 2 === 0 ? '' : 'bg-slate-50/40'}`}>
                          ...
                          {meses.map(mo => {
                            const cell = row.cells[mo] ?? { vol: 0, lbPct: 0, margPct: 0, mediaDias: 0, medianaDias: 0 };
                            return (
