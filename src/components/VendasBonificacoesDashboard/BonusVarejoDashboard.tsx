...
async function exportToExcel(rows: BonusVarejoRow[], sheetName: string, filename: string): Promise<void> {
  ...
  const labels = COLS_VAREJO.map(c => c.label);
  ...
  rows.forEach((row, ri) => {
    ...
    const values = COLS_VAREJO.map(col => {
