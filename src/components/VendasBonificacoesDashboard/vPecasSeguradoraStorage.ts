...
const KEY_VPECAS_SEG = 'registro_vpecas_seg';
const KEY_VPECAS_SEG_DEVOLUCAO = 'registro_vpecas_seg_devolucao';
...
const KEY_PERIOD_META  = (p: string) => `registro_vpecas_seg_per_${p}_meta`;
const KEY_PERIOD_CHUNK = (p: string, i: number) => `registro_vpecas_seg_per_${p}_c${i}`;

async function savePeriodRows(periodo: string, rows: VPecasSegRow[]): Promise<void> {
  const chunks: VPecasSegRow[][] = [];
...
async function loadPeriodRows(periodo: string): Promise<VPecasSegRow[]> {
  ...
  const chunks = await Promise.all(keys.map(k => kvGet<VPecasSegRow[]>(k)));
  return chunks.filter((c): c is VPecasSegRow[] => Array.isArray(c)).flat();
}

async function getAllSegPeriods(): Promise<string[]> {
  try {
    const keys = await kvKeys('registro_vpecas_seg_per_*_meta');
    return keys.map(k => k.replace('registro_vpecas_seg_per_', '').replace('_meta', ''));
  } catch { return []; }
}

// ─── CRUD ─────────────────────────────────────────────────────────────────────
export async function loadVPecasSegRows(): Promise<VPecasSegRow[]> {
  try {
    const periods = await getAllSegPeriods();
    if (periods.length > 0) {
      const arrays = await Promise.all(periods.map(loadPeriodRows));
      return arrays.flat();
    }
    const rawData = await kvGet(KEY_VPECAS_SEG);
    if (!Array.isArray(rawData)) return [];
    return (rawData as Record<string, unknown>[]).map(item => {
      if (item.data && typeof item.data === 'object') return item as unknown as VPecasSegRow;
...
export async function saveVPecasSegRows(rows: VPecasSegRow[]): Promise<boolean> {
  try {
    const byPeriod = new Map<string, VPecasSegRow[]>();
    for (const r of rows) {
    ...
    const existingPeriods = await getAllSegPeriods();
...
export async function appendVPecasSegRows(
  newRows: Omit<VPecasSegRow, 'id'>[],
): Promise<{ added: number }> {
  const byPeriod = new Map<string, Omit<VPecasSegRow, 'id'>[]>();
...
export async function replaceVPecasSegRows(
  rows: Omit<VPecasSegRow, 'id'>[],
): Promise<{ total: number }> {
  const periods = await getAllSegPeriods();
  await Promise.all(periods.map(p => savePeriodRows(p, [])));
  try { await kvSet(KEY_VPECAS_SEG, []); } catch { /* legacy */ }
  const { added } = await appendVPecasSegRows(rows);
  return { total: added };
}
...
export async function loadVPecasSegDevolucaoRows(): Promise<VPecasSegRow[]> {
  try {
    const rawData = await kvGet(KEY_VPECAS_SEG_DEVOLUCAO);
    if (!Array.isArray(rawData)) return [];
    return (rawData as Record<string, unknown>[]).map(item => {
      if (item.data && typeof item.data === 'object') return { ...(item as unknown as VPecasSegRow), isDevolucao: true };
...
export async function saveVPecasSegDevolucaoRows(rows: VPecasSegRow[]): Promise<boolean> {
  try {
    await kvSet(KEY_VPECAS_SEG_DEVOLUCAO, rows);
    return true;
  } catch {
    return false;
  }
...
export async function appendVPecasSegDevolucaoRows(
  periodo: string,
  newRows: Omit<VPecasSegRow, 'id'>[],
): Promise<{ added: number; removed: number }> {
  const existing = await loadVPecasSegDevolucaoRows();
  const kept = existing.filter(r => r.periodoImport !== periodo);
  const removed = existing.length - kept.length;
  const toAdd: VPecasSegRow[] = newRows.map(r => ({ ...r, id: crypto.randomUUID(), periodoImport: periodo, isDevolucao: true }));
  await saveVPecasSegDevolucaoRows([...kept, ...toAdd]);
...
// ─── Parser TXT ───────────────────────────────────────────────────────────────
export function parsePecasSegTxt(content: string): Omit<VPecasSegRow, 'id'>[] {
  ...
  const result: Omit<VPecasSegRow, 'id'>[] = [];
...
export function parsePecasSegDevolucaoTxt(content: string): Omit<VPecasSegRow, 'id'>[] {
...
// ─── Parser Excel ─────────────────────────────────────────────────────────────
export function parsePecasSegExcel(buffer: ArrayBuffer): Omit<VPecasSegRow, 'id'>[] {
