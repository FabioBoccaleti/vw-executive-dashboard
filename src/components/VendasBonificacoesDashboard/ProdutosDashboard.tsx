...
import { Download, FileSpreadsheet, ChevronDown, AlertTriangle, Pencil, Trash2, Highlighter, StickyNote, Check, X, Plus, Tag } from 'lucide-react';
...
import {
  loadProdutosRows,
  saveProdutosRows,
  loadProdutosMonitoradosCodes,
  saveProdutosMonitoradosCodes,
} from './produtosMonitoradosStorage';
...
async function exportToExcel(rows: VPecasItemRow[], allCols: string[], filename: string): Promise<void> {
  ...
  wb.created = new Date();
  const ws = wb.addWorksheet('Produtos Monitorados', {
  ...
  const titleRow = ws.addRow([`Produtos Monitorados - ${today}`]);
...
export function ProdutosDashboard() {
  ...
  // Gerenciamento de códigos monitorados
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([loadProdutosRows(), loadProdutosMonitoradosCodes()]).then(([data, codes]) => {
      setRows(data);
  ...
  async function handleToggleHighlight(row: VPecasItemRow) {
    ...
    setRows(updated);
    await saveProdutosRows(updated);
  }
  ...
  async function handleAnnotationBlur() {
    await saveProdutosRows(rows);
  }

  async function handleSaveEditModal() {
    if (!editValues || !editModal) return;
    const updated = rows.map(r => r.id === editModal.id ? { ...r, data: editValues } : r);
    setRows(updated);
    await saveProdutosRows(updated);
    setEditModal(null);
    setEditValues(null);
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `produtos_monitorados_${filterYear}_${monthLabel}.xlsx`);
  ...
  async function handleAddCode() {
    ...
    if (!code) return;
    if (monitoredCodes.some(c => c.toUpperCase() === code)) {
      toast.warning('Código já está na lista.');
      return;
    }
    const updated = [...monitoredCodes, code];
    setMonitoredCodes(updated);
    await saveProdutosMonitoradosCodes(updated);
    setNewCode('');
    toast.success(`Código ${code} adicionado. Será incluído na próxima importação.`);
  }

  async function handleRemoveCode(code: string) {
    const updated = monitoredCodes.filter(c => c !== code);
    setMonitoredCodes(updated);
    await saveProdutosMonitoradosCodes(updated);
    setConfirmDeleteCode(null);
    toast.success(`Código ${code} removido.`);
  }
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
      ...
      {/* Banner informativo */}
      <div className="mx-4 mt-3 flex items-start gap-2 rounded-lg border border-violet-200 bg-violet-50 px-4 py-2.5 text-xs text-violet-800">
        <span className="mt-0.5 shrink-0">ℹ️</span>
        <span>
          <strong>Dados sincronizados automaticamente</strong> a partir da importação TXT de <strong>Itens de Peças</strong>.
          Os itens são capturados do arquivo completo (sem o filtro 100+100) com base nos códigos monitorados abaixo.
        </span>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              <AlertTriangle className="w-5 h-5 text-red-500 mt-0.5 flex-shrink-0" />
              <div>
                <p className="font-semibold text-slate-800 text-sm">Remover código monitorado</p>
                <p className="text-slate-500 text-xs mt-1">Remover <strong>{confirmDeleteCode}</strong> da lista? Os dados já importados não serão excluídos.</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <button className="text-xs border border-slate-200 rounded px-3 py-1.5 hover:bg-slate-50" onClick={() => setConfirmDeleteCode(null)}>Cancelar</button>
              <button className="text-xs bg-red-600 hover:bg-red-700 text-white rounded px-3 py-1.5" onClick={() => handleRemoveCode(confirmDeleteCode)}>Remover</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-3 flex items-center gap-3 flex-shrink-0 flex-wrap">
        <Button
          ...
          onClick={() => setShowManageCodes(v => !v)}
        >
          <Tag className="w-4 h-4" />
          Códigos Monitorados
          <span className="ml-1 bg-violet-100 text-violet-700 rounded-full px-1.5 py-0.5 text-[10px] font-bold">
        ...
        <span className="text-xs text-slate-400">
          Atualizado via importação em Itens de Peças · {allColumns.length} colunas
        </span>
      </div>

      {/* Painel de gerenciamento de códigos */}
      {showManageCodes && (
        <div className="bg-violet-50 border-b border-violet-100 px-6 py-4 flex-shrink-0">
          <p className="text-xs font-semibold text-violet-800 mb-3">
            Códigos ITEM_ESTOQUE_PUB monitorados — itens com esses códigos são capturados automaticamente ao importar o TXT em <strong>Itens de Peças</strong>.
          </p>
          <div className="flex flex-wrap gap-2 mb-3">
            {monitoredCodes.map(code => (
              <div key={code} className="flex items-center gap-1 bg-white border border-violet-200 rounded-full px-3 py-1 text-xs font-mono font-semibold text-violet-800 shadow-sm">
                {code}
                <button
            ...
            {monitoredCodes.length === 0 && (
              <span className="text-xs text-slate-400 italic">Nenhum código monitorado. Adicione abaixo.</span>
            )}
          </div>
          <div className="flex items-center gap-2 max-w-xs">
            <input
              type="text"
              className="flex-1 border border-violet-300 rounded-lg px-3 py-1.5 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-violet-400 uppercase"
              placeholder="Ex: 6EA096301A"
              value={newCode}
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6 flex-shrink-0 flex-wrap">
        ...
        <span className="text-xs text-slate-500">
          Venda: <span className="font-semibold text-violet-700 font-mono">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-slate-300 gap-2">
            ...
            <span className="text-sm">
              {rows.length === 0
                ? 'Nenhum dado — importe um arquivo TXT em Itens de Peças para sincronizar'
                : 'Nenhum item para o período selecionado'}
            </span>
          </div>
        ) : (
          <table className="w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10">
              <tr className="bg-violet-700 text-white">
                ...
                {allColumns.map(col => (
                  <th key={col} className="bg-violet-700 px-3 py-2.5 text-left font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRows.map((row, i) => {
                const bgColor = row.highlight ? '#f5f3ff' : i % 2 === 0 ? '#ffffff' : '#f8fafc';
                return (
                  <Fragment key={row.id}>
                    <tr className={`${
                      row.highlight ? 'bg-violet-50 border-l-4 border-l-violet-400' : i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'
                      ...
                      <td className="px-2 py-1.5 whitespace-nowrap sticky left-0 z-4"
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          <button onClick={() => handleToggleHighlight(row)} title="Evidenciar"
                            className={`p-1.5 rounded transition-colors ${row.highlight ? 'text-violet-500 bg-violet-50' : 'text-slate-300 hover:text-violet-400 hover:bg-violet-50'}`}>
                      ...
                      <tr key={`${row.id}-annot`} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td colSpan={allColumns.length + 1} className="px-4 pb-2">
                          <textarea
                            ...
                            rows={2} placeholder="Adicione uma anotação..."
