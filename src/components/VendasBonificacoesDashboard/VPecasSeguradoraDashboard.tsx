...
import {
  ...
  loadVPecasSegRows,
  appendVPecasSegRows,
  replaceVPecasSegRows,
  saveVPecasSegRows,
  parsePecasSegTxt,
  parsePecasSegExcel,
  loadVPecasSegDevolucaoRows,
  saveVPecasSegDevolucaoRows,
  appendVPecasSegDevolucaoRows,
  parsePecasSegDevolucaoTxt,
} from './vPecasSeguradoraStorage';
...
function getRowPeriod(r: VPecasSegRow): { year: number; month: number } | null {
  if (r.periodoImport) {
...
function sumField(rows: VPecasSegRow[], key: string): number {
...
// ─── Exportar Excel ───────────────────────────────────────────────────────────
async function exportToExcel(rows: VPecasSegRow[], allCols: string[], filename: string): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Peças Seg. Balcão', {
  ...
  const titleRow = ws.addRow([`Peças Seguradora Balcão - ${today}`]);
...
export function VPecasSeguradoraDashboard() {
  const [rows, setRows]               = useState<VPecasSegRow[]>([]);
  const [devolucaoRows, setDevolucaoRows] = useState<VPecasSegRow[]>([]);
  ...
  const [editModal, setEditModal]                 = useState<VPecasSegRow | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([loadVPecasSegRows(), loadVPecasSegDevolucaoRows()]).then(([data, devol]) => {
      setRows(data);
  ...
  async function handleAnnotationBlur() {
    await saveVPecasSegRows(rows);
    await saveVPecasSegDevolucaoRows(devolucaoRows);
  }
  ...
  async function handleDeleteAll() {
    const periodFilter = (r: VPecasSegRow) => {
      const d = getRowPeriod(r);
      if (!d) return true;
    ...
    await saveVPecasSegRows(keptVendas);
    await saveVPecasSegDevolucaoRows(keptDevol);
  ...
  async function handleTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasSegTxt(text);
    if (parsed.length === 0) {
    ...
    const { added } = await appendVPecasSegRows(parsed.map(r => ({ ...r, periodoImport: periodo })));
    const updated = await loadVPecasSegRows();
    setRows(updated);
  ...
  async function handleDevolucaoTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const text = await file.text();
    const parsed = parsePecasSegDevolucaoTxt(text);
    if (parsed.length === 0) {
    ...
    const { added, removed } = await appendVPecasSegDevolucaoRows(periodo, parsed);
    const updated = await loadVPecasSegDevolucaoRows();
    setDevolucaoRows(updated);
    const msg = removed > 0
  ...
  async function handleXlsxImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasSegExcel(buffer);
    if (parsed.length === 0) {
      toast.warning('Nenhum registro encontrado no Excel.');
      if (xlsxInputRef.current) xlsxInputRef.current.value = '';
      return;
    }
    const { total } = await replaceVPecasSegRows(parsed);
    const updated = await loadVPecasSegRows();
    setRows(updated);
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `vpecas_seguradora_${filterYear}_${monthLabel}.xlsx`);
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-5">
              <Upload className="w-5 h-5 text-sky-600 mt-0.5 flex-shrink-0" />
              <div>
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="flex-1">
                ...
                <select value={importPeriodMonth} onChange={e => setImportPeriodMonth(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar Excel — Peças Seguradora Balcão</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                {allColumns.map(col => (
                  <div key={col}>
                    ...
                    <input
                      className="w-full border border-slate-300 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-sky-300"
            ...
            <div className="flex justify-end gap-2 mt-4 flex-shrink-0">
              ...
              <button className="text-xs bg-sky-600 hover:bg-sky-700 text-white rounded px-3 py-1.5 flex items-center gap-1.5"
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-3 flex items-center gap-3 flex-shrink-0 flex-wrap">
        <Button size="sm" className="flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white"
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-2 flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <div className="relative mr-2">
          <select value={filterYear} onChange={e => setFilterYear(Number(e.target.value))}
            className="appearance-none text-sm font-bold text-slate-700 border border-slate-200 rounded-lg pl-3 pr-7 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-sky-400 cursor-pointer">
        ...
        <button onClick={() => setFilterMonth(null)}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterMonth === null ? 'bg-sky-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}>
          Ano todo
        </button>
        {MONTHS.map((name, idx) => {
          const m = idx + 1;
          ...
          return (
            <button key={m} onClick={() => hasData ? setFilterMonth(m) : undefined}
              className={`relative px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                isActive ? 'bg-sky-600 text-white shadow-sm' : hasData ? 'text-slate-600 hover:bg-slate-100' : 'text-slate-300 cursor-default'
              }`}>
              {name}
              {hasData && (
                <span className={`absolute -top-1.5 -right-1.5 text-[9px] font-bold px-1 rounded-full leading-none py-0.5 ${
                  isActive ? 'bg-white text-sky-700' : 'bg-sky-100 text-sky-600'
                }`}>{count}</span>
              )}
            </button>
          );
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6 flex-shrink-0 flex-wrap">
        ...
        <span className="text-xs text-slate-500">
          Total NF: <span className="font-semibold text-sky-700 font-mono">
            {kpis['TOT_NOTA_FISCAL']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Líquido: <span className="font-semibold text-emerald-700 font-mono">
          ...
          <table className="w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10">
              <tr className="bg-sky-700 text-white">
                <th className="sticky left-0 z-20 bg-sky-700 px-3 py-2.5 text-center font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap w-20">
                  Ações
                </th>
                {allColumns.map(col => (
                  <th key={col}
                    className="bg-sky-700 px-3 py-2.5 text-left font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
              ...
              {filteredRows.map((row, i) => {
                ...
                return (
                  <Fragment key={row.id}>
                    ...
                    } border-b border-slate-100 hover:bg-sky-50/30 transition-colors`}>
                      <td className="px-2 py-1.5 whitespace-nowrap sticky left-0 z-4"
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          ...
                          <button onClick={() => { setEditModal(row); setEditValues({ ...row.data }); }} title="Editar"
                            className="p-1.5 rounded text-slate-300 hover:text-sky-600 hover:bg-sky-50 transition-colors">
                        ...
                        if (col === 'MODALIDADE') {
                          return (
                            <td key={col} className="px-3 py-1.5 whitespace-nowrap text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${val === 'G' ? 'bg-amber-100 text-amber-700' : 'bg-sky-100 text-sky-700'}`}>
                                {val || '-'}
                              </span>
                            </td>
                          );
                    ...
                    {expandedAnnotations.has(row.id) && (
                      <tr key={`${row.id}-annot`} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td colSpan={allColumns.length + 1} className="px-4 pb-2">
                          <textarea
                            className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-300 resize-none bg-sky-50/60"
