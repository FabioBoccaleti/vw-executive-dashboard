...
import type { VPecasEPecasRow as VPecasRow } from './vPecasEPecasStorage';
...
// autoTaxaEP: quando fornecido, sobrescreve taxaEPecas do override (calculado via lookup Taxa E-Peças)
function calcPecasRow(d: Record<string, string>, ov: PecasOverride, autoTaxaEP?: number): PecasCalc {
...
export default function VPecasEPecasVendasDashboard() {
  ...
  useEffect(() => {
    Promise.all([loadVPecasEPecasRows(), loadVPecasEPecasDevolucaoRows(), loadOverrides(), loadTaxaEPecasRows()]).then(([rows, devol, ov, taxaRows]) => {
      ...
      setOverrides(ov);
      setTaxaEPRows(taxaRows as TaxaEPecasRow[]);
    });
  }, []);
  ...
  // Taxa E-Peças: lookup por TITULO == NUMERO_NOTA_FISCAL
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
      ...
      <div className="mx-4 mb-2 mt-1 flex items-start gap-2 rounded-lg border border-teal-200 bg-teal-50 px-4 py-2.5 text-xs text-teal-800">
        ...
        <span>
          Dados importados via <strong>Registros → Registro de Vendas → Peças E-Peças</strong>.
          Taxa E-Peças calculada via <strong>Registros → Taxa E-Peças</strong>.
              ...
              {filteredRows.length === 0 && (
                <tr>
                  <td colSpan={28} className="text-center py-16 text-slate-300 text-sm">
                    Nenhum registro — importe dados de Peças E-Peças na aba Registros
                  </td>
                </tr>
              )}
