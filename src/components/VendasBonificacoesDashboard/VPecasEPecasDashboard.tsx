...
import {
  type VPecasEPecasRow,
  NF_HEADERS,
  CURRENCY_FIELDS,
  DATE_FIELDS,
  loadVPecasEPecasRows,
  appendVPecasEPecasRows,
  replaceVPecasEPecasRows,
  saveVPecasEPecasRows,
  parsePecasEPecasTxt,
  parsePecasEPecasExcel,
  loadVPecasEPecasDevolucaoRows,
  saveVPecasEPecasDevolucaoRows,
  appendVPecasEPecasDevolucaoRows,
  parsePecasEPecasDevolucaoTxt,
} from './vPecasEPecasStorage';
...
function getRowPeriod(r: VPecasEPecasRow): { year: number; month: number } | null {
  if (r.periodoImport) {
...
function sumField(rows: VPecasEPecasRow[], key: string): number {
...
// ─── Exportar Excel ───────────────────────────────────────────────────────────
async function exportToExcel(rows: VPecasEPecasRow[], allCols: string[], filename: string): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Peças E-Peças', {
  ...
  const titleRow = ws.addRow([`Peças E-Peças - ${today}`]);
...
export function VPecasEPecasDashboard() {
  const [rows, setRows]               = useState<VPecasEPecasRow[]>([]);
  const [devolucaoRows, setDevolucaoRows] = useState<VPecasEPecasRow[]>([]);
  ...
  const [editModal, setEditModal]                 = useState<VPecasEPecasRow | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([loadVPecasEPecasRows(), loadVPecasEPecasDevolucaoRows()]).then(([data, devol]) => {
      setRows(data);
  ...
  async function handleAnnotationBlur() {
    await saveVPecasEPecasRows(rows);
    await saveVPecasEPecasDevolucaoRows(devolucaoRows);
  }
  ...
  async function handleDeleteAll() {
    const periodFilter = (r: VPecasEPecasRow) => {
      const d = getRowPeriod(r);
      if (!d) return true;
    ...
    await saveVPecasEPecasRows(keptVendas);
    await saveVPecasEPecasDevolucaoRows(keptDevol);
  ...
  async function handleTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasEPecasTxt(text);
    if (parsed.length === 0) {
    ...
    const { added } = await appendVPecasEPecasRows(parsed.map(r => ({ ...r, periodoImport: periodo })));
    const updated = await loadVPecasEPecasRows();
    setRows(updated);
  ...
  async function handleDevolucaoTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const text = await file.text();
    const parsed = parsePecasEPecasDevolucaoTxt(text);
    if (parsed.length === 0) {
    ...
    const { added, removed } = await appendVPecasEPecasDevolucaoRows(periodo, parsed);
    const updated = await loadVPecasEPecasDevolucaoRows();
    setDevolucaoRows(updated);
    const msg = removed > 0
  ...
  async function handleXlsxImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasEPecasExcel(buffer);
    if (parsed.length === 0) {
      toast.warning('Nenhum registro encontrado no Excel.');
      if (xlsxInputRef.current) xlsxInputRef.current.value = '';
      return;
    }
    const { total } = await replaceVPecasEPecasRows(parsed);
    const updated = await loadVPecasEPecasRows();
    setRows(updated);
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `vpecas_epeças_${filterYear}_${monthLabel}.xlsx`);
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar TXT — Período?</p>
                <p className="text-slate-500 text-xs mt-1">Os registros serão classificados no mês/ano informado.</p>
              </div>
            </div>
            <div className="flex items-center gap-3 mb-5">
              <div className="flex-1">
                ...
                <select value={importPeriodMonth} onChange={e => setImportPeriodMonth(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar Excel — Peças E-Peças</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                {allColumns.map(col => (
                  <div key={col}>
                    ...
                    <input
                      className="w-full border border-slate-300 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-300"
            ...
            <div className="flex justify-end gap-2 mt-4 flex-shrink-0">
              ...
              <button className="text-xs bg-indigo-600 hover:bg-indigo-700 text-white rounded px-3 py-1.5 flex items-center gap-1.5"
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-2 flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <div className="relative mr-2">
          <select value={filterYear} onChange={e => setFilterYear(Number(e.target.value))}
            className="appearance-none text-sm font-bold text-slate-700 border border-slate-200 rounded-lg pl-3 pr-7 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400 cursor-pointer">
        ...
        <button onClick={() => setFilterMonth(null)}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterMonth === null ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}>
          Ano todo
        </button>
        {MONTHS.map((name, idx) => {
          const m = idx + 1;
          ...
          return (
            <button key={m} onClick={() => hasData ? setFilterMonth(m) : undefined}
              className={`relative px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                isActive ? 'bg-indigo-600 text-white shadow-sm' : hasData ? 'text-slate-600 hover:bg-slate-100' : 'text-slate-300 cursor-default'
              }`}>
              {name}
              {hasData && (
                <span className={`absolute -top-1.5 -right-1.5 text-[9px] font-bold px-1 rounded-full leading-none py-0.5 ${
                  isActive ? 'bg-white text-indigo-700' : 'bg-indigo-100 text-indigo-600'
                }`}>{count}</span>
              )}
            </button>
          );
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6 flex-shrink-0 flex-wrap">
        ...
        <span className="text-xs text-slate-500">
          Total NF: <span className="font-semibold text-indigo-700 font-mono">
            {kpis['TOT_NOTA_FISCAL']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Líquido: <span className="font-semibold text-indigo-700 font-mono">
            {kpis['LIQ_NOTA_FISCAL']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Custo Médio: <span className="font-semibold text-slate-700 font-mono">
          ...
          <table className="w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10">
              <tr className="bg-indigo-700 text-white">
                <th className="sticky left-0 z-20 bg-indigo-700 px-3 py-2.5 text-center font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap w-20">
                  Ações
                </th>
                {allColumns.map(col => (
                  <th key={col}
                    className="bg-indigo-700 px-3 py-2.5 text-left font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
                      ...
                      <td className="px-2 py-1.5 whitespace-nowrap sticky left-0 z-4"
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          ...
                          <button onClick={() => handleToggleAnnotation(row.id)} title="Anotação"
                            className={`p-1.5 rounded transition-colors ${row.annotation ? 'text-indigo-500 bg-indigo-50' : 'text-slate-300 hover:text-indigo-400 hover:bg-indigo-50'}`}>
                        ...
                        if (col === 'MODALIDADE') {
                          return (
                            <td key={col} className="px-3 py-1.5 whitespace-nowrap text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${val === 'G' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                {val || '-'}
                              </span>
                            </td>
                          );
                    ...
                    {expandedAnnotations.has(row.id) && (
                      <tr key={`${row.id}-annot`} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td colSpan={allColumns.length + 1} className="px-4 pb-2">
                          <textarea
                            className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none bg-indigo-50/60"
