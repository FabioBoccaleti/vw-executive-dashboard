...
function getRowPeriod(r: TaxaMLRow): { year: number; month: number } | null {
  if (r.periodoImport) {
    const [y, m] = r.periodoImport.split('-').map(Number);
    if (y && m) return { year: y, month: m };
  }
  // Tenta detectar uma coluna de data nos dados
...
// ─── Exportar Excel ───────────────────────────────────────────────────────────
async function exportToExcel(rows: TaxaMLRow[], allCols: string[], filename: string): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Taxa Mercado Livre', {
  ...
  const titleRow = ws.addRow([`Taxa Mercado Livre - ${today}`]);
...
export function TaxaMercadoLivreDashboard() {
  const [rows, setRows]               = useState<TaxaMLRow[]>([]);
  ...
  const [editModal, setEditModal]                 = useState<TaxaMLRow | null>(null);
  const [editValues, setEditValues]               = useState<Record<string, string> | null>(null);
  const [expandedAnnotations, setExpandedAnnotations] = useState<Set<string>>(new Set());

  // Detecta todas as colunas dinamicamente a partir dos dados importados
  ...
  // ─── Ações ──────────────────────────────────────────────────────────────────
  async function handleToggleHighlight(row: TaxaMLRow) {
    const updated = rows.map(r => r.id === row.id ? { ...r, highlight: !r.highlight } : r);
    setRows(updated);
    await saveTaxaMLRows(updated);
  }
  ...
  async function handleTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parseTaxaMLTxt(text);
    if (parsed.length === 0) {
    ...
    const { added } = await appendTaxaMLRows(parsed.map(r => ({ ...r, periodoImport: periodo })));
    const updated = await loadTaxaMLRows();
    setRows(updated);
  ...
  async function handleXlsxImport(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    // Lê Excel e converte dinamicamente
    ...
    const parsed: Omit<TaxaMLRow, 'id'>[] = raw.map(r =>
      ({ data: Object.fromEntries(Object.entries(r).map(([k, v]) => [k, String(v)])) })
    );
    const { total } = await replaceTaxaMLRows(parsed);
    const updated = await loadTaxaMLRows();
    setRows(updated);
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `taxa_mercado_livre_${filterYear}_${monthLabel}.xlsx`);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar TXT — Taxa Mercado Livre</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar Excel — Taxa Mercado Livre</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                {allColumns.map(col => (
                  <div key={col}>
                    ...
                    <input
                      className="w-full border border-slate-300 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-orange-300"
            ...
            <div className="flex justify-end gap-2 mt-4 flex-shrink-0">
              ...
              >Cancelar</button>
              <button
                className="text-xs bg-orange-600 hover:bg-orange-700 text-white rounded px-3 py-1.5 flex items-center gap-1.5"
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-2 flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <div className="relative mr-2">
          <select
            ...
            className="appearance-none text-sm font-bold text-slate-700 border border-slate-200 rounded-lg pl-3 pr-7 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-orange-400 cursor-pointer"
          >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            filterMonth === null ? 'bg-orange-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'
          }`}
        >
          Ano todo
        </button>
            ...
            >
              ...
              {hasData && (
                <span className={`absolute -top-1.5 -right-1.5 text-[9px] font-bold px-1 rounded-full leading-none py-0.5 ${
                  isActive ? 'bg-white text-orange-700' : 'bg-orange-100 text-orange-600'
                }`}>
                  {count}
                </span>
              )}
                ...
                <tr className="bg-orange-700 text-white">
                  {allColumns.map((h, i) => (
                    <th
                      ...
                      className="bg-orange-700 px-3 py-2.5 text-left font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap border-r border-orange-600"
                    >
                      {h}
                    </th>
                  ))}
                  <th className="sticky right-0 z-20 bg-orange-700 px-3 py-2.5 text-center font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap w-24">
                    Ações
                  </th>
                </tr>
              </thead>
                ...
                {filteredRows.map((row, i) => (
                  <Fragment key={row.id}>
                    <tr
                      ...
                      } border-b border-slate-100 hover:bg-orange-50/40 transition-colors`}
                    >
