...
export function CentralVendasResumoPage() {
  ...
  const handleShareWhatsApp = async () => {
    const el = document.querySelector<HTMLElement>('.resumo-print-area');
    if (!el) {
      setWaMsg('⚠️ Elemento não encontrado na página.');
      setTimeout(() => setWaMsg(null), 5000);
      return;
    }

    // Abre WhatsApp Web AGORA (síncrono, antes de qualquer await — evita bloqueio de popup)
    window.open('https://web.whatsapp.com/', '_blank');

    setSharingWA(true);
    try {
      // Expande o elemento para capturar conteúdo completo (sem corte por scroll)
      const prevOverflow = el.style.overflow;
      const prevHeight = el.style.height;
      el.style.overflow = 'visible';
      el.style.height = 'auto';

      // dom-to-image-more suporta oklch e CSS moderno
      ...
      // Gera PDF com jsPDF usando as dimensões reais da imagem capturada
      const img = new Image();
      img.src = dataUrl;
      ...
      const pdf = new jsPDF({ orientation, unit: 'px', format: [img.width / 1.5, img.height / 1.5] });
      pdf.addImage(dataUrl, 'JPEG', 0, 0, img.width / 1.5, img.height / 1.5);

      // Download via <a> (pdf.save() pode ser bloqueado após window.open)
      const pdfDataUrl = pdf.output('datauristring');
      const a = document.createElement('a');
      a.href = pdfDataUrl;
      a.download = `Resumo-Executivo-${currLabel.replace('/', '-')}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);

      setWaMsg('PDF salvo nos Downloads! No WhatsApp Web, clique no clipe 📎 → Documento e anexe o arquivo.');
      setTimeout(() => setWaMsg(null), 10000);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setWaMsg(`⚠️ Erro ao gerar PDF: ${msg}`);
      setTimeout(() => setWaMsg(null), 10000);
    } finally {
      setSharingWA(false);
    }
    ...
    <div className="flex-1 flex flex-col bg-slate-50 overflow-auto" id="resumo-executivo-print">
      ...
      <div className="print-hidden bg-white border-b border-slate-200 px-6 py-3 flex flex-wrap items-center gap-4 shrink-0">
        ...
        <button
          ...
          disabled={sharingWA}
          className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-xs font-semibold hover:bg-green-500 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {sharingWA ? (
            ...
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
              ...
              <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.124 1.525 5.857L.057 23.5l5.784-1.517A11.95 11.95 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.894a9.888 9.888 0 01-5.034-1.378l-.36-.214-3.733.979.997-3.648-.235-.374A9.86 9.86 0 012.106 12C2.106 6.58 6.58 2.106 12 2.106S21.894 6.58 21.894 12 17.42 21.894 12 21.894z"/>
            </svg>
          )}
          {sharingWA ? 'Gerando…' : 'WhatsApp'}
        </button>
        <button
          onClick={handlePrint}
      ...
      {/* ── Banner de instrução WhatsApp ──────────────────────────────────── */}
      {waMsg && (
        <div className="print-hidden mx-6 mt-3 flex items-start gap-3 bg-green-50 border border-green-200 text-green-800 rounded-xl px-4 py-3 text-sm font-medium shadow-sm">
          <svg className="w-5 h-5 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.124 1.525 5.857L.057 23.5l5.784-1.517A11.95 11.95 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.894a9.888 9.888 0 01-5.034-1.378l-.36-.214-3.733.979.997-3.648-.235-.374A9.86 9.86 0 012.106 12C2.106 6.58 6.58 2.106 12 2.106S21.894 6.58 21.894 12 17.42 21.894 12 21.894z"/>
          </svg>
          <span>{waMsg}</span>
          <button onClick={() => setWaMsg(null)} className="ml-auto text-green-500 hover:text-green-700 font-bold text-base leading-none">×</button>
        </div>
      )}
