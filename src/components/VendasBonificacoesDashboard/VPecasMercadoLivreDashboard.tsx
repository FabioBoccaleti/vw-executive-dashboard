...
import {
  ...
  parsePecasMLTxt,
  parsePecasMLExcel,
  loadVPecasMLDevolucaoRows,
  saveVPecasMLDevolucaoRows,
  appendVPecasMLDevolucaoRows,
  parsePecasMLDevolucaoTxt,
} from './vPecasMercadoLivreStorage';
...
function getRowPeriod(r: VPecasMLRow): { year: number; month: number } | null {
  if (r.periodoImport) {
...
function sumField(rows: VPecasMLRow[], key: string): number {
...
// ─── Exportar Excel ───────────────────────────────────────────────────────────
async function exportToExcel(rows: VPecasMLRow[], allCols: string[], filename: string): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Peças Mercado Livre', {
  ...
  const titleRow = ws.addRow([`Peças Mercado Livre - ${today}`]);
...
export function VPecasMercadoLivreDashboard() {
  const [rows, setRows]               = useState<VPecasMLRow[]>([]);
  const [devolucaoRows, setDevolucaoRows] = useState<VPecasMLRow[]>([]);
  ...
  const [editModal, setEditModal]                 = useState<VPecasMLRow | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([loadVPecasMLRows(), loadVPecasMLDevolucaoRows()]).then(([data, devol]) => {
      setRows(data);
  ...
  async function handleAnnotationBlur() {
    await saveVPecasMLRows(rows);
    await saveVPecasMLDevolucaoRows(devolucaoRows);
  }
  ...
  async function handleDeleteAll() {
    const periodFilter = (r: VPecasMLRow) => {
      const d = getRowPeriod(r);
      if (!d) return true;
    ...
    await saveVPecasMLRows(keptVendas);
    await saveVPecasMLDevolucaoRows(keptDevol);
  ...
  async function handleTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasMLTxt(text);
    if (parsed.length === 0) {
    ...
    const { added } = await appendVPecasMLRows(parsed.map(r => ({ ...r, periodoImport: periodo })));
    const updated = await loadVPecasMLRows();
    setRows(updated);
  ...
  async function handleDevolucaoTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const text = await file.text();
    const parsed = parsePecasMLDevolucaoTxt(text);
    if (parsed.length === 0) {
    ...
    const { added, removed } = await appendVPecasMLDevolucaoRows(periodo, parsed);
    const updated = await loadVPecasMLDevolucaoRows();
    setDevolucaoRows(updated);
    const msg = removed > 0
  ...
  async function handleXlsxImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasMLExcel(buffer);
    if (parsed.length === 0) {
      toast.warning('Nenhum registro encontrado no Excel.');
      if (xlsxInputRef.current) xlsxInputRef.current.value = '';
      return;
    }
    const { total } = await replaceVPecasMLRows(parsed);
    const updated = await loadVPecasMLRows();
    setRows(updated);
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `vpecas_mercadolivre_${filterYear}_${monthLabel}.xlsx`);
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar Excel — Peças Mercado Livre</p>
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6 flex-shrink-0 flex-wrap">
        ...
        <span className="text-xs text-slate-500">
          Total NF: <span className="font-semibold text-emerald-700 font-mono">
                      ...
                      <td className="px-2 py-1.5 whitespace-nowrap sticky left-0 z-4"
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          ...
                          <button onClick={() => handleToggleAnnotation(row.id)} title="Anotação"
                            className={`p-1.5 rounded transition-colors ${row.annotation ? 'text-emerald-500 bg-emerald-50' : 'text-slate-300 hover:text-emerald-400 hover:bg-emerald-50'}`}>
                        ...
                        if (col === 'MODALIDADE') {
                          return (
                            <td key={col} className="px-3 py-1.5 whitespace-nowrap text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${val === 'G' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                                {val || '-'}
                              </span>
                            </td>
                          );
                    ...
                    {expandedAnnotations.has(row.id) && (
                      <tr key={`${row.id}-annot`} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td colSpan={allColumns.length + 1} className="px-4 pb-2">
                          <textarea
                            className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-300 resize-none bg-emerald-50/60"
