...
function EditCell({ value, type, onSave }: { value: string; type: 'text' | 'currency'; onSave: (v: string) => void }) {
    ...
    return (
      <div className="flex items-center gap-1 min-w-0">
        <input
          ...
          className="border border-violet-400 rounded px-1 py-0.5 text-xs w-full focus:outline-none focus:ring-1 focus:ring-violet-500"
        />
        <button onClick={commit} className="text-violet-600 hover:text-violet-700 flex-shrink-0"><Check className="w-3 h-3" /></button>
  ...
  return (
    <div
      className="group flex items-center justify-between gap-1 cursor-pointer hover:bg-violet-50 rounded px-1 py-0.5 min-w-0"
      onClick={() => { setDraft(value); setEditing(true); }}
    >
      <span className={`truncate text-xs ${type === 'currency' ? 'font-mono' : ''}`}>{display}</span>
      <Pencil className="w-2.5 h-2.5 text-slate-300 group-hover:text-violet-500 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
    </div>
  );
}
    ...
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                filterMonth === null ? 'bg-violet-600 text-white' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
              Ano todo
            </button>
            {MONTHS.map((m, i) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                  filterMonth === i + 1 ? 'bg-violet-600 text-white' : 'text-slate-500 hover:bg-slate-100'
      ...
      <div className="mx-4 mb-2 mt-1 flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs text-blue-800">
        ...
        <span>
          ...
          <span className="font-mono">V21, U21, I21, C41, C21, V42, V29, V23, U25, U27, P64, P65, P66, P68, P37, P30, G23, P27, O25, ST1, A52</span>{' '}
          ...
          <table className="w-full border-separate border-spacing-0 text-xs" style={{ minWidth: 2800 }}>
            <thead className="sticky top-0 z-10">
              ...
              <tr>
                ...
                <th colSpan={5}  className="bg-violet-800 text-white text-center py-1.5 text-[10px] font-semibold tracking-wide border-r border-violet-700">RECEITA BRUTA</th>
              ...
              <tr className="text-[10px] font-bold text-white">
                ...
                {['Valor Venda', 'ICMS', 'PIS', 'COFINS', 'Difal'].map((h, i) => (
                  <th key={i} className="bg-violet-800 px-3 py-2 text-right whitespace-nowrap border-b border-violet-700 border-r border-violet-700">{h}</th>
                ))}
                {/* REC. LÍQUIDA */}
            ...
            <tbody>
              {filteredRows.length === 0 && (
                <tr>
                  <td colSpan={26} className="text-center py-16 text-slate-300 text-sm">
                    Nenhum registro — importe dados de V. Peças na aba Registros
                  </td>
                </tr>
              )}
              {filteredRows.map((row, ri) => {
                ...
                return (
                  <tr key={row.id} className="group transition-colors hover:bg-violet-50/30">
                    {/* IDENTIFICAÇÃO */}
                    <td
