...
import {
  type VPecasItemRow,
  ITEM_HEADERS,
  ITEM_CURRENCY_FIELDS,
  ITEM_PERCENT_FIELDS,
  ITEM_DATE_FIELDS,
  loadVPecasItemRows,
  importVPecasItemForPeriod,
  replaceVPecasItemRows,
  saveVPecasItemRows,
  parsePecasItemTxt,
  parsePecasItemExcel,
  loadVPecasItemDevolucaoRows,
  saveVPecasItemDevolucaoRows,
...
function getRowPeriod(r: VPecasItemRow): { year: number; month: number } | null {
  if (r.periodoImport) {
    const [y, m] = r.periodoImport.split('-').map(Number);
    if (y && m) return { year: y, month: m };
  }
  return parseDate(r.data['DTA_ENTRADA_SAIDA'] ?? '');
}
...
function sumFieldNum(rows: VPecasItemRow[], key: string): number {
...
function colType(key: string): 'currency' | 'percent' | 'date' | 'text' {
  if (ITEM_CURRENCY_FIELDS.has(key)) return 'currency';
  if (ITEM_PERCENT_FIELDS.has(key)) return 'percent';
  if (ITEM_DATE_FIELDS.has(key)) return 'date';
  return 'text';
}

async function exportToExcel(rows: VPecasItemRow[], allCols: string[], filename: string): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Itens Pecas', {
  ...
  const titleRow = ws.addRow([`Vendas de Pecas por Item - ${today}`]);
  ...
  rows.forEach((row, ri) => {
    ...
    const values = allCols.map(h => {
      const v = row.data[h] ?? '';
      if (ITEM_CURRENCY_FIELDS.has(h)) return parseFloat(v.replace(',', '.')) || null;
      if (ITEM_PERCENT_FIELDS.has(h)) return parseFloat(v.replace(',', '.')) || null;
      if (ITEM_DATE_FIELDS.has(h)) {
    ...
    dr.eachCell({ includeEmpty: true }, (cell, ci) => {
      ...
      if (!h) return;
      if (ITEM_CURRENCY_FIELDS.has(h)) {
        cell.numFmt = BRL_FMT;
        cell.alignment = { horizontal: 'right', vertical: 'middle' };
        cell.font = { size: 9, name: 'Courier New' };
      } else if (ITEM_PERCENT_FIELDS.has(h)) {
        cell.numFmt = PCT_FMT;
        cell.alignment = { horizontal: 'right', vertical: 'middle' };
        cell.font = { size: 9 };
      } else if (ITEM_DATE_FIELDS.has(h)) {
...
export function VPecasItemDashboard() {
  const [rows, setRows]               = useState<VPecasItemRow[]>([]);
  const [devolucaoRows, setDevolucaoRows] = useState<VPecasItemRow[]>([]);
  ...
  const [editModal, setEditModal]                 = useState<VPecasItemRow | null>(null);
  const [editValues, setEditValues]               = useState<Record<string, string> | null>(null);
  const [expandedAnnotations, setExpandedAnnotations] = useState<Set<string>>(new Set());

  useEffect(() => {
    setLoading(true);
    Promise.all([loadVPecasItemRows(), loadVPecasItemDevolucaoRows()]).then(([data, devol]) => {
      setRows(data);
  ...
  const allColumns = useMemo(() => {
    ...
    allRows.forEach(r => Object.keys(r.data).forEach(k => {
      if (!ITEM_HEADERS.includes(k as never)) extras.add(k);
    }));
    return [...ITEM_HEADERS, ...Array.from(extras)];
  }, [allRows]);
  ...
  const kpis = useMemo(() =>
    Object.fromEntries(KPI_FIELDS.map(k => [k, sumFieldNum(filteredRows, k)])),
    [filteredRows]
  );

  async function handleToggleHighlight(row: VPecasItemRow) {
    if (row.isDevolucao) {
      const updated = devolucaoRows.map(r => r.id === row.id ? { ...r, highlight: !r.highlight } : r);
      setDevolucaoRows(updated);
      await saveVPecasItemDevolucaoRows(updated);
    } else {
      const updated = rows.map(r => r.id === row.id ? { ...r, highlight: !r.highlight } : r);
      setRows(updated);
      await saveVPecasItemRows(updated);
    }
  }
  ...
  function handleToggleAnnotation(id: string) {
    setExpandedAnnotations(prev => { const n = new Set(prev); if (n.has(id)) n.delete(id); else n.add(id); return n; });
  }
  ...
  async function handleAnnotationBlur() {
    await saveVPecasItemRows(rows);
    await saveVPecasItemDevolucaoRows(devolucaoRows);
  }

  async function handleSaveEditModal() {
    if (!editModal || !editValues) return;
    if (editModal.isDevolucao) {
      const updated = devolucaoRows.map(r => r.id === editModal.id ? { ...r, data: { ...r.data, ...editValues } } : r);
      setDevolucaoRows(updated);
      await saveVPecasItemDevolucaoRows(updated);
    } else {
      const updated = rows.map(r => r.id === editModal.id ? { ...r, data: { ...r.data, ...editValues } } : r);
      setRows(updated);
      await saveVPecasItemRows(updated);
    }
    setEditModal(null); setEditValues(null);
    toast.success('Linha salva.');
  }

  async function handleDeleteAll() {
    const periodFilter = (r: VPecasItemRow) => {
      const d = getRowPeriod(r);
      if (!d) return true;
    ...
    await saveVPecasItemRows(keptVendas);
    await saveVPecasItemDevolucaoRows(keptDevol);
    setConfirmDeleteAll(false);
    toast.success(`Itens de ${MONTHS[(filterMonth ?? 1) - 1]}/${filterYear} excluidos.`);
  }
  ...
  async function handleTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasItemTxt(text);
    if (parsed.length === 0) {
  ...
  async function handleDevolucaoTxtImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const text = await file.text();
    const parsed = parsePecasItemDevolucaoTxt(text);
    if (parsed.length === 0) {
      toast.warning('Nenhum item de devolução (P07/A07) reconhecido no arquivo.');
      if (devolTxtRef.current) devolTxtRef.current.value = '';
      return;
    }
    const periodo = `${devolPeriodYear}-${String(devolPeriodMonth).padStart(2, '0')}`;
    const { added, removed, originalCount } = await importVPecasItemDevolucaoForPeriod(periodo, parsed);
    const updated = await loadVPecasItemDevolucaoRows();
    setDevolucaoRows(updated);
    const filteredOut = originalCount - added;
    const filterNote = filteredOut > 0 ? ` (${filteredOut} filtrado(s) por rentabilidade)` : '';
    const msg = removed > 0
      ? `${added} devolução(ões) importada(s) de ${originalCount}${filterNote} · ${removed} substituida(s) — ${MONTHS[devolPeriodMonth - 1]}/${devolPeriodYear}.`
      : `${added} devolução(ões) importada(s) de ${originalCount}${filterNote} — ${MONTHS[devolPeriodMonth - 1]}/${devolPeriodYear}.`;
    toast.success(msg);
    if (devolTxtRef.current) devolTxtRef.current.value = '';
  }
  ...
  async function handleXlsxImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const parsed = parsePecasItemExcel(buffer);
    if (parsed.length === 0) {
  ...
  async function handleExport() {
    try {
      ...
      await exportToExcel(filteredRows, allColumns, `vpecas_item_${filterYear}_${monthLabel}.xlsx`);
    ...
    <div className="flex-1 flex flex-col" style={{ minHeight: 0 }}>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-1">Se ja houver dados neste mes, serao substituidos.</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-1">Os valores monetários serão importados como negativos. Se já houver devoluções neste mês, serão substituídas.</p>
              </div>
            </div>
            <div className="flex items-center gap-3 mb-5">
              <div className="flex-1">
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-slate-500 text-xs mt-1">
                  Excluir todos os itens de <strong>{MONTHS[(filterMonth ?? 1) - 1]}/{filterYear}</strong>?
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-800 text-sm">Importar Excel — Vendas de Pecas por Item</p>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between mb-4 flex-shrink-0">
              <p className="font-semibold text-slate-800 text-sm">
                Editar item — {editModal.data['DES_ITEM_ESTOQUE'] || editModal.data['ITEM_ESTOQUE'] || editModal.id}
              </p>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                {allColumns.map(col => (
                  <div key={col}>
                    ...
                    <input
                      className="w-full border border-slate-300 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-300"
            ...
            <div className="flex justify-end gap-2 mt-4 flex-shrink-0">
              ...
              <button className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white rounded px-3 py-1.5 flex items-center gap-1.5"
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-3 flex items-center gap-3 flex-shrink-0 flex-wrap">
        ...
        <span className="text-xs text-slate-400">
          TXT: substitui o mes · Excel: substitui tudo · {allColumns.length} colunas
        </span>
      </div>
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-2 flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <button onClick={() => setFilterMonth(null)}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterMonth === null ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}>
          Ano todo
        </button>
        {MONTHS.map((name, idx) => {
          const m = idx + 1;
          ...
          return (
            <button key={m} onClick={() => hasData ? setFilterMonth(m) : undefined}
              className={`relative px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                isActive ? 'bg-emerald-600 text-white shadow-sm' : hasData ? 'text-slate-600 hover:bg-slate-100' : 'text-slate-300 cursor-default'
              }`}>
              {name}
              {hasData && (
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center gap-6 flex-shrink-0 flex-wrap">
        <span className="text-xs text-slate-500">
          <span className="font-semibold text-slate-700">{filteredRows.length}</span> item(s)
        </span>
        <span className="text-xs text-slate-500">
          Venda: <span className="font-semibold text-emerald-700 font-mono">
            {kpis['VAL_VENDA']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Rentabilidade: <span className="font-semibold text-teal-700 font-mono">
            {kpis['VAL_RENTABILIDADE']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Impostos: <span className="font-semibold text-slate-700 font-mono">
            {kpis['VAL_IMPOSTOS']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Desconto: <span className="font-semibold text-amber-700 font-mono">
            {kpis['VAL_DESCONTO']?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </span>
        </span>
        <span className="text-xs text-slate-500">
          Qtde: <span className="font-semibold text-slate-700 font-mono">
            {kpis['QUANTIDADE']?.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}
          </span>
        </span>
      </div>
      ...
      <div ref={tableRef} className="flex-1 overflow-auto" style={{ minHeight: 0 }}>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-slate-300 gap-2">
            ...
            <span className="text-sm">Nenhum item — importe um arquivo TXT ou Excel</span>
          </div>
        ) : (
          <table className="w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10">
              <tr className="bg-emerald-700 text-white">
                <th className="sticky left-0 z-20 bg-emerald-700 px-3 py-2.5 text-center font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap w-20">
                  Acoes
                </th>
                {allColumns.map(col => (
                  <th key={col} className="bg-emerald-700 px-3 py-2.5 text-left font-semibold text-[10px] uppercase tracking-wide whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRows.map((row, i) => {
                const bgColor = row.highlight ? '#ecfdf5' : i % 2 === 0 ? '#ffffff' : '#f8fafc';
                return (
                  <Fragment key={row.id}>
                    <tr className={`${
                      row.highlight ? 'bg-emerald-50 border-l-4 border-l-emerald-400' : i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'
                      ...
                      <td className="px-2 py-1.5 whitespace-nowrap sticky left-0 z-4"
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          <button onClick={() => handleToggleHighlight(row)} title="Evidenciar"
                            className={`p-1.5 rounded transition-colors ${row.highlight ? 'text-emerald-500 bg-emerald-50' : 'text-slate-300 hover:text-emerald-400 hover:bg-emerald-50'}`}>
                          ...
                          <button onClick={() => { setEditModal(row); setEditValues({ ...row.data }); }} title="Editar"
                            className="p-1.5 rounded text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 transition-colors">
                      ...
                      {allColumns.map(col => {
                        ...
                        if (col === 'CLASS_ABC') {
                          const color = val === 'A' ? 'bg-emerald-100 text-emerald-700' : val === 'B' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600';
                          return (
                        ...
                        if (col === 'MODALIDADE') {
                          return (
                            <td key={col} className="px-3 py-1.5 whitespace-nowrap text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${val === 'V' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                                {val || '-'}
                              </span>
                            </td>
                          );
                    ...
                    {expandedAnnotations.has(row.id) && (
                      <tr key={`${row.id}-annot`} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td colSpan={allColumns.length + 1} className="px-4 pb-2">
                          <textarea
                            className="w-full text-xs border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-300 resize-none bg-sky-50/60"
