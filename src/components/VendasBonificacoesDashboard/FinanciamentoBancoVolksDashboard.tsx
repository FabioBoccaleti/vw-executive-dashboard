...
const DEMO_CAMPO_LABELS: Record<CampoAssinaturaComissao, string> = {
...
export function FinanciamentoBancoVolksDashboard({ onBack }: Props) {
  ...
  function handleAbrirDemoAssinatura(vendor: string, campo: CampoAssinaturaComissao) {
    if (!session) return;
    setDemoAssinaDialog({ vendor, campo, senha: '', loading: false, erro: null });
  }

  async function handleConfirmarDemoAssinatura() {
    if (!demoAssinaDialog || !session) return;
    setDemoAssinaDialog(prev => prev ? { ...prev, loading: true, erro: null } : prev);
    const result = await apiLogin(session.username, demoAssinaDialog.senha);
    if ('error' in result) {
      setDemoAssinaDialog(prev => prev ? { ...prev, loading: false, erro: 'Senha incorreta. Tente novamente.' } : prev);
      return;
    }
    const assinatura: AssinaturaDigital = {
      username: session.username,
    ...
    const next: Record<string, Partial<Record<CampoAssinaturaComissao, AssinaturaDigital>>> = {
      ...demoAssinaturas,
      [demoAssinaDialog.vendor]: {
        ...(demoAssinaturas[demoAssinaDialog.vendor] ?? {}),
        [demoAssinaDialog.campo]: assinatura,
      },
    };
    setDemoAssinaturas(next);
    await kvSet(`financiamento:assinaturas:${vendasYear}:${vendasMonth}`, next);
    toast.success(`Assinatura de ${DEMO_CAMPO_LABELS[demoAssinaDialog.campo]} registrada!`);
    setDemoAssinaDialog(null);
  }

  function handlePrintDemonstrativo() {
    const printArea = document.querySelector<HTMLElement>('.demo-print-area');
    if (!printArea) return;
...
<head>
  <meta charset="UTF-8"/>
  <title>Demonstrativo de Pagamento \u2014 ${MONTHS[vendasMonth - 1]}/${vendasYear}</title>
  ...
  <style>
    ...
    @media print {
      ...
      .demo-page:last-of-type { page-break-after: avoid; break-after: avoid; }
...
<body>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
                ...
                <div className="overflow-auto max-h-[calc(100vh-280px)] print:overflow-visible print:max-h-none">
                      ...
                      <div key={vi} className="demo-page bg-white border border-slate-200 rounded-xl p-5 mb-4">
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-200">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {(Object.entries(DEMO_CAMPO_LABELS) as [CampoAssinaturaComissao, string][]).map(([campo, label]) => {
                              const ass = demoAssinaturas[vendor]?.[campo];
                              return (
                                <div key={campo} className="flex flex-col gap-1.5">
                                  ...
                                  {ass ? (
                                    ...
                                    <button
                                      onClick={() => handleAbrirDemoAssinatura(vendor, campo)}
                                      className="border-2 border-dashed border-slate-300 rounded-lg px-4 py-3 text-sm text-slate-400 hover:border-teal-400 hover:text-teal-600 hover:bg-teal-50 transition-colors flex items-center gap-2 justify-center print-hidden"
                                    >
                  ...
                  {vendors.length > 0 && (
                    <div className="demo-page bg-white border border-slate-200 rounded-xl p-5 mb-4">
                      ...
                      {/* Campos de Assinatura (Resumo Geral) */}
                      <div className="mt-6 pt-4 border-t border-slate-200">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">Assinaturas</p>
                        <div className="grid grid-cols-2 gap-4">
                          {(Object.entries(DEMO_CAMPO_LABELS) as [CampoAssinaturaComissao, string][]).map(([campo, label]) => {
                            const ass = demoAssinaturas['__resumo__']?.[campo];
                            return (
                              <div key={campo} className="flex flex-col gap-1.5">
                                <p className="text-xs text-slate-500 font-medium">{label}</p>
                                {ass ? (
                                  ...
                                  <button
                                    onClick={() => handleAbrirDemoAssinatura('__resumo__', campo)}
                                    className="border-2 border-dashed border-slate-300 rounded-lg px-4 py-3 text-sm text-slate-400 hover:border-teal-400 hover:text-teal-600 hover:bg-teal-50 transition-colors flex items-center gap-2 justify-center print-hidden"
                                  >
                                    <PenLine className="w-4 h-4" />
                                    Assinar
      ...
      {/* ── Dialog: Assinatura Eletrônica (Demonstrativo Banco Volks) ── */}
      {demoAssinaDialog && (
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800">
                  Assinar — {DEMO_CAMPO_LABELS[demoAssinaDialog.campo]}
                </h3>
              </div>
              <button onClick={() => setDemoAssinaDialog(null)} className="text-slate-400 hover:text-slate-600 p-1 rounded hover:bg-slate-100">✕</button>
            </div>
            <div className="px-5 py-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  type="password"
                  autoFocus
                  value={demoAssinaDialog.senha}
                  onChange={e => setDemoAssinaDialog(prev => prev ? { ...prev, senha: e.target.value, erro: null } : prev)}
                  onKeyDown={e => e.key === 'Enter' && !demoAssinaDialog.loading && handleConfirmarDemoAssinatura()}
                  className="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400"
                  placeholder="Digite sua senha"
                />
                {demoAssinaDialog.erro && (
                  <p className="text-xs text-red-500 mt-0.5">{demoAssinaDialog.erro}</p>
                )}
              </div>
            </div>
            <div className="px-5 py-4 border-t border-slate-100 flex justify-end gap-2">
              <button
                onClick={() => setDemoAssinaDialog(null)}
                className="px-4 py-2 rounded-lg border border-slate-200 text-sm text-slate-500 hover:bg-slate-50"
              >
                Cancelar
              </button>
              <button
                onClick={handleConfirmarDemoAssinatura}
                disabled={demoAssinaDialog.loading || !demoAssinaDialog.senha}
