...
export function PeliculasDemonstrativo({ rows, onPagar, onClose }: Props) {
  ...
  async function handleCancelarAssinatura() {
    ...
    const result = await apiLogin(session.username, cancelarAssinaturaDialog.senha);
    if ('error' in result) {
      setCancelarAssinaturaDialog(prev => prev ? { ...prev, loading: false, erro: 'Senha incorreta. Tente novamente.' } : prev);
      return;
    }
    ...
    const next = { ...assinaturas };
    if (Object.keys(nextPessoa).length > 0) next[cancelarAssinaturaDialog.pessoa] = nextPessoa;
    else delete next[cancelarAssinaturaDialog.pessoa];
