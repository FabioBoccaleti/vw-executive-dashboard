...
import {
  ...
  Banknote, CheckCircle2, AlertCircle, Printer, Calculator, NotebookPen,
} from 'lucide-react';
import { toast } from 'sonner';
import {
  loadEsteticaRows, saveEsteticaRows, createEmptyEsteticaRow,
  recalcEsteticaRow, type EsteticaRow,
} from './esteticaStorage';
import {
  loadEsteticaVendedores, loadEsteticaVendedoresAcessorios, loadEsteticaProdutos,
  loadEsteticaAliquotas, loadEsteticaRegras, type RegraRemuneracao,
} from '@/components/CadastrosPage/cadastrosStorage';
import ExcelJS from 'exceljs';
import { saveAs } from 'file-saver';
import * as XLSX from 'xlsx';
import { EsteticaAnalise } from './EsteticaAnalise';
import { EsteticaDemonstrativo } from './EsteticaDemonstrativo';
...
const REQUIRED_KEYS: (keyof EsteticaRow)[] = [
...
interface ColDef { key: keyof EsteticaRow; label: string; type: ColType; width: number; }
...
// ─── Export to Excel ──────────────────────────────────────────────────────────
async function exportExcel(exportRows: EsteticaRow[]): Promise<void> {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Sorana Executive Dashboard';
  wb.created = new Date();
  const ws = wb.addWorksheet('Estética Audi', {
  ...
  const titleRow = ws.addRow([`Análise e Controle de Vendas de Estética na Audi — ${today}`]);
...
// ─── Filter helpers ───────────────────────────────────────────────────────────
type FilterValues = Partial<Record<keyof EsteticaRow, string>>;
function rowMatchesFilters(row: EsteticaRow, filters: FilterValues): boolean {
  for (const [key, term] of Object.entries(filters) as [keyof EsteticaRow, string][]) {
    if (!term) continue;
...
// ─── Main Component ───────────────────────────────────────────────────────────
interface EsteticaDashboardProps {
  onBack: () => void;
  onOpenCadastros?: () => void;
}

export function EsteticaDashboard({ onBack, onOpenCadastros }: EsteticaDashboardProps) {
  const [rows, setRows]           = useState<EsteticaRow[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editDraft, setEditDraft] = useState<EsteticaRow | null>(null);
  ...
  const canTabela = isAdmin() || canAccessVendasSub('estetica.tabela');
  const canAnalise = isAdmin() || canAccessVendasSub('estetica.analise');
  const [importPreview, setImportPreview] = useState<EsteticaRow[] | null>(null);
  ...
  const [liberarNFRows, setLiberarNFRows] = useState<EsteticaRow[]>([]);
  ...
  const handlePagarComissoes = async (updatedRows: EsteticaRow[]) => {
    setRows(updatedRows);
    const ok = await persist(updatedRows);
    if (ok) toast.success('Comissão(s) marcada(s) como paga(s).');
  };
  ...
  useEffect(() => {
    loadEsteticaRows().then(r => { setRows(r); setLoading(false); });
    loadEsteticaVendedores().then(list => {
    ...
    loadEsteticaRegras().then(r => setRegras(r));
  }, []);
  ...
  const persist = async (updated: EsteticaRow[]): Promise<boolean> => {
    setSaving(true);
    try {
      const ok = await saveEsteticaRows(updated);
  ...
  const startEdit = (row: EsteticaRow) => {
    setDeleteId(null);
    setEditingId(row.id);
    setEditDraft(recalcEsteticaRow({ ...row }, aliquotaTotal, comissaoCtx));
  };
  ...
  const insertAt = async (index: number) => {
    const row = createEmptyEsteticaRow();
    const updated = [...rows];
  ...
  const openObsPopover = (row: EsteticaRow) => {
    setObsPopoverId(row.id);
    setObsDraft(row.observacao ?? '');
    setTimeout(() => obsTextareaRef.current?.focus(), 50);
  };

  const saveObs = async (id: string) => {
    const updated = rows.map(r => r.id === id ? { ...r, observacao: obsDraft } : r);
    setRows(updated);
    setObsPopoverId(null);
    await persist(updated);
...
<body>
  <div class="header">
    <div><h1>✓ PAGAMENTO LIBERADO</h1><p>Análise e Controle de Vendas de Estética na Audi — Comprovante de Liberação de Pagamento</p></div>
  ...
  <div class="footer">Documento gerado em ${new Date().toLocaleString('pt-BR')} — Sistema Executive Dashboard · Análise e Controle de Vendas de Estética na Audi</div>
</body></html>`;
  ...
  const registerVenda = async () => {
    ...
    const base = createEmptyEsteticaRow();
    const newRow = recalcEsteticaRow({
      ...base,
  ...
  const setFilter = (key: keyof EsteticaRow, value: string) =>
  ...
  const handleImportFile = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imported: EsteticaRow[] = dataRaws.map(row => {
        const draft = createEmptyEsteticaRow();
        ...
        return recalcEsteticaRow(draft, aliquotaTotal, comissaoCtx);
      });
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold leading-tight tracking-tight">
                Análise e Controle de Vendas de Estética na Audi
              </h1>
                    ...
                    return (
                      <Fragment key={row.id}>
                        <tr data-row-id={row.id} style={{ background: rowBg }} className="transition-colors group/row">
                          ...
                          {/* Data cells */}
                          {visibleColumns.map((col, ci) => {
                            const val = (draft as EsteticaRow)[col.key] as string;
                            ...
                            const isRequired = REQUIRED_KEYS.includes(col.key as keyof EsteticaRow);
                              ...
                              {col.key === 'situacao' && (() => {
                                ...
                                return (
                                  <td key="pago" className="border-r border-slate-100 px-2 py-2.5 text-center text-xs bg-emerald-50" style={{ verticalAlign: 'middle' }}>
                                    ...
                                    {hasA && <div className="text-emerald-700 font-semibold whitespace-nowrap">S: {fmtDate(aPago)}</div>}
                                  </td>
                                );
                              })()}
                              </Fragment>
                            ...
                            {isDelete ? (
                              ...
                              <div className="flex items-center justify-center gap-0.5">
                                {row.situacao === 'Processo Finalizado' && !unlockedIds.has(row.id) ? (
                                  <>
                                    <button
                                      onClick={() => openObsPopover(row)}
                                      title="Observações"
                                      className={`relative p-1.5 rounded-md transition-colors ${row.observacao?.trim() ? 'text-indigo-500 hover:bg-indigo-50' : 'text-slate-300 hover:text-indigo-400 hover:bg-indigo-50'}`}
                                    >
                                      <NotebookPen className="w-3.5 h-3.5" />
                                      {row.observacao?.trim() && <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 bg-indigo-500 rounded-full" />}
                                    </button>
                                    <button
      ...
      {/* ── Modal Observações ── */}
      {obsPopoverId && (() => {
        const row = rows.find(r => r.id === obsPopoverId);
        const label = row?.numeroOS?.trim() ? `OS ${row.numeroOS}` : row?.nomeCliente?.trim() ? row.nomeCliente : 'Registro';
        return (
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setObsPopoverId(null)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col gap-4" onClick={e => e.stopPropagation()}>
              <div className="flex items-start gap-3">
                <div className="p-2.5 bg-indigo-100 rounded-xl flex-shrink-0">
                  <NotebookPen className="w-5 h-5 text-indigo-700" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-800">Observações</h3>
                  <p className="text-xs text-slate-400 mt-0.5">{label}</p>
                </div>
                <button onClick={() => setObsPopoverId(null)} className="ml-auto p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <textarea
                ...
                onKeyDown={e => { if (e.key === 'Escape') setObsPopoverId(null); }}
                rows={5}
                placeholder="Digite sua observação aqui..."
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none text-slate-700 placeholder:text-slate-300"
              />
              <div className="flex gap-3 justify-end">
                <Button variant="outline" size="sm" onClick={() => setObsPopoverId(null)} className="border-slate-300 text-slate-600">Cancelar</Button>
                <Button size="sm" onClick={() => saveObs(obsPopoverId!)} className="bg-indigo-600 hover:bg-indigo-700 text-white gap-1.5">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-white flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">Estética Audi</p>
                  </div>
                </div>
                {!liberarNFLiberado && (
