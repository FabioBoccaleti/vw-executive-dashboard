...
function applyComissaoAutoFill(rows: VendasResultadoRow[], modalidade: RemuneracaoModalidade): VendasResultadoRow[] {
  ...
  return rows.map(row => {
    ...
    if (vv === 0) return row;
    const comBruta = vv * (parseFloat(String(row.pctComissao).replace(',', '.')) || 0) / 100;
    const lb = comBruta - n(row.impostos); // Com. Líquida (direta)
    ...
    const pct3 = parseFloat(String(modalidade.comissaocomLiquida).replace(',', '.')) || 0;
...
// ─── Cálculo por linha ────────────────────────────────────────────────────────
function calcDireta(r: VendasResultadoRow, aliqBon: number, dsrPct: number) {
  const recLiq = n(r.valorVenda); // Receita Líquida = Valor de Venda (direta)
...
function agg(rows: VendasResultadoRow[], aliqBon: number, dsrCfg: VendasDsrConfig[]): Agg | null {
  ...
  const v07 = rows.filter(r => false /* VD/Frotista sem devoluções */).length;
  ...
  for (const r of rows) {
    ...
    const c = calcDireta(r, aliqBon, d);
...
const MODEL_METRIC_CFG: Record<ModelMetric, {
  ...
  lb:     { label: 'Com. Líquida',  donutKey: 'lb',     barKey: 'lb',     barLabel: 'Com. Líquida',  fmt: fmtBRL,                    icon: <DollarSign className="w-3 h-3" /> },
...
async function exportExcel(
  ...
  const addSheet = (name: string, tab: string, headers: string[], rows: (string | number)[][], widths: number[]) => {
    ...
    const t = ws.addRow([`Análise VD / Frotista — ${periodLabel} — ${name}`]);
  ...
  addSheet('Vendedores', 'FF3B82F6', ['Vendedor', 'Vol. Líq.', 'Resultado (R$)', 'Margem %', 'Com. Líquida (R$)', 'LB %', 'Rem. Vendedor (R$)'],
    vendorData.map(v => [v.name, v.vol, v.res, v.marg, v.lb, v.lbPct, v.remVendedor]), [28, 10, 18, 12, 18, 12, 18]);
  addSheet('Evolução Mensal', 'FF10B981', ['Mês', 'Vol. Líq.', 'Receita Líq.', 'Com. Líquida', 'Lucro c/ Bôn.', 'Resultado', 'Margem %'],
    monthlyData.map(m => [m.label, m.vol, m.recLiq, m.lb, m.lcb, m.res, m.marg]), [10, 10, 18, 18, 18, 18, 12]);
  addSheet('Mix por Marca VD', 'FFF59E0B', ['Modelo', 'Vol.', 'Com. Líquida', 'Margem %', 'Resultado'],
    modelData.map(m => [m.name, m.vol, m.lb, m.marg, m.res]), [22, 10, 18, 12, 18]);
  const buf = await wb.xlsx.writeBuffer();
  saveAs(new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
    `analise-vd-frotista-${periodLabel.toLowerCase().replace(/\//g, '-')}-${new Date().toISOString().split('T')[0]}.xlsx`);
}

// ═══════════════════════════════════════════════════════════════════════════════
// COMPONENTE PRINCIPAL
// ═══════════════════════════════════════════════════════════════════════════════
export function VendasDiretaAnalise() {
  ...
  const devolucoes = useMemo(() => filteredRows.filter(r => false /* VD/Frotista sem devoluções */), [filteredRows]);
  ...
  const daysInMonthDireta = useMemo(() => {
    if (month === null) return [];
    return Array.from({ length: new Date(year, month, 0).getDate() }, (_, i) => i + 1);
  }, [year, month]);

  const dailyDiretaData = useMemo(() => {
    if (month === null) return [];
    ...
    return daysInMonthDireta.map(day => {
  ...
  }, [filteredRows, month, dailyVendedor, dailyFamilia, daysInMonthDireta]);

  // ── Evolução Mensal (usado quando month === null / "Todos") ───────────────
  const monthlyDiretaData = useMemo(() => {
  ...
  const coresData = useMemo(() => {
    ...
    for (const r of filteredRows) {
      ...
      const c = calcDireta(r, aliqBon, dsrPct);
  ...
  const waterfallData = useMemo(() => {
    ...
    push('Com. c/ Bônus', metrics.lcb, 'pos');
  ...
  const kpiDefs = useMemo(() => {
    ...
    return [
      ...
      { id: 'comLiquida', label: 'Com. Líquida', value: fmtBRL(metrics.lb), sub: fmtPct(metrics.lbPct) + ' da receita',
      ...
      { id: 'totalBon',   label: 'Total de Bônus (líquido)', value: fmtBRL(metrics.bon * (1 - aliqBon / 100)),
        ...
        info: metrics.bon === 0 ? 'PIV/SIQ/PIVE não preenchidos na aba Vendas → VD / Frotista' : undefined },
      ...
      { id: 'juros',      label: 'Juros de Estoque', value: fmtBRL(metrics.juros),
        ...
        info: 'Campo "Juros Estoque" — preencher manualmente em Vendas → VD / Frotista' },
      { id: 'mediaDias',  label: 'Giro Médio (dias)', value: metrics.mediaDias > 0 ? metrics.mediaDias.toFixed(0) + ' dias' : '—',
        ...
        accent: '#06b6d4',
        info: 'Campo "Dias Estoque" — preencher manualmente em Vendas → VD / Frotista' },
    ];
  }, [metrics, prevMetrics]);
      ...
      {!loading && metrics && (
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            {month === null ? (
              monthlyDiretaData.every(d => d.receita === 0 && d.qtd === 0) ? (
                <div className="h-24 flex items-center justify-center text-slate-300 text-xs">Sem dados no período</div>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={monthlyDiretaData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              dailyDiretaData.every(d => d.receita === 0 && d.qtd === 0) ? (
                <div className="h-24 flex items-center justify-center text-slate-300 text-xs">Sem dados no período</div>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={dailyDiretaData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                    ...
                    <Tooltip content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white border border-slate-200 rounded-xl shadow-xl px-4 py-3 text-xs min-w-[190px]">
                          ...
                          <div className="flex justify-between gap-4"><span className="text-slate-400">Com. Líquida</span><span className="font-mono text-indigo-600">{fmtBRLF(d.lb)}</span></div>
                        </div>
                      );
                    }} />
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <ResponsiveContainer width="100%" height={290}>
                <ComposedChart data={monthlyData} margin={{ top: 8, right: 58, left: 8, bottom: 0 }} barCategoryGap="22%" barGap={3}>
                  ...
                  <Tooltip content={({ active, payload, label }: { active?: boolean; payload?: { payload?: { recLiq: number; lb: number; res: number } }[]; label?: string }) => {
                    ...
                    return (
                      <div className="bg-white border border-slate-200 rounded-xl shadow-xl px-4 py-3.5 text-xs min-w-[240px]" style={{ boxShadow: '0 8px 24px rgba(0,0,0,0.10)' }}>
                        ...
                        <div className="flex justify-between items-center gap-4 py-1 border-b border-slate-100">
                          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-indigo-500 inline-block" />Com. Líquida</span>
                  ...
                  <Bar yAxisId="left" dataKey="lb" name="Com. Líquida" radius={[4, 4, 0, 0]}>
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-1.5 justify-center mt-3 text-[10.5px] text-slate-500">
                ...
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-indigo-500 inline-block" />Com. Líquida</span>
                    ...
                    return (
                      <div key={v.name} className={`rounded-2xl border-2 p-5 relative overflow-hidden ${bg}`}>
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                          ...
                          <div>
                            <p className="text-slate-400 text-[10px]">Com. Líquida</p>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200">
                            {['#', 'Vendedor', 'Vol.', '% Grupo', 'Com. Líquida', 'Resultado', 'Margem', 'Custo Rem. Vendedor'].map(h => (
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {([
                    ...
                    { label: 'Com. Líquida',        get: (m: Agg) => m.lb,            fmt: fmtBRLF, hl: false },
                    { label: '% Com. Líquida',     get: (m: Agg) => m.lbPct,        fmt: (v: number) => fmtPct(v), hl: false },
                    { label: 'Ticket Médio',       get: (m: Agg) => m.ticketReceita, fmt: (v: number, pm: Agg) => pm.netVol > 0 ? fmtBRLF(v) : '—', hl: false },
                    { label: 'Com. c/ Bônus',     get: (m: Agg) => m.lcb,          fmt: fmtBRLF, hl: false },
