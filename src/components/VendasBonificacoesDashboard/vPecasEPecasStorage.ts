...
const KEY_VPECAS_EP          = 'registro_vpecas_ep';
const KEY_VPECAS_EP_DEVOLUCAO = 'registro_vpecas_ep_devolucao';
...
// ─── Tipo principal ───────────────────────────────────────────────────────────
export interface VPecasEPecasRow {
  id: string;
...
const KEY_PERIOD_META  = (p: string) => `registro_vpecas_ep_per_${p}_meta`;
const KEY_PERIOD_CHUNK = (p: string, i: number) => `registro_vpecas_ep_per_${p}_c${i}`;

async function savePeriodRows(periodo: string, rows: VPecasEPecasRow[]): Promise<void> {
  const chunks: VPecasEPecasRow[][] = [];
...
async function loadPeriodRows(periodo: string): Promise<VPecasEPecasRow[]> {
  ...
  const chunks = await Promise.all(keys.map(k => kvGet<VPecasEPecasRow[]>(k)));
  return chunks.filter((c): c is VPecasEPecasRow[] => Array.isArray(c)).flat();
}

async function getAllEPPeriods(): Promise<string[]> {
  try {
    const keys = await kvKeys('registro_vpecas_ep_per_*_meta');
    return keys.map(k => k.replace('registro_vpecas_ep_per_', '').replace('_meta', ''));
  } catch { return []; }
}

// ─── CRUD ─────────────────────────────────────────────────────────────────────
export async function loadVPecasEPecasRows(): Promise<VPecasEPecasRow[]> {
  try {
    const periods = await getAllEPPeriods();
    if (periods.length > 0) {
      const arrays = await Promise.all(periods.map(loadPeriodRows));
      return arrays.flat();
    }
    const rawData = await kvGet(KEY_VPECAS_EP);
    if (!Array.isArray(rawData)) return [];
    return (rawData as Record<string, unknown>[]).map(item => {
      if (item.data && typeof item.data === 'object') return item as unknown as VPecasEPecasRow;
...
export async function saveVPecasEPecasRows(rows: VPecasEPecasRow[]): Promise<boolean> {
  try {
    const byPeriod = new Map<string, VPecasEPecasRow[]>();
    for (const r of rows) {
    ...
    const existingPeriods = await getAllEPPeriods();
...
export async function appendVPecasEPecasRows(
  newRows: Omit<VPecasEPecasRow, 'id'>[],
): Promise<{ added: number }> {
  const byPeriod = new Map<string, Omit<VPecasEPecasRow, 'id'>[]>();
...
export async function replaceVPecasEPecasRows(
  rows: Omit<VPecasEPecasRow, 'id'>[],
): Promise<{ total: number }> {
  const periods = await getAllEPPeriods();
  await Promise.all(periods.map(p => savePeriodRows(p, [])));
  try { await kvSet(KEY_VPECAS_EP, []); } catch { /* legacy */ }
  const { added } = await appendVPecasEPecasRows(rows);
  return { total: added };
}
...
export async function loadVPecasEPecasDevolucaoRows(): Promise<VPecasEPecasRow[]> {
  try {
    const rawData = await kvGet(KEY_VPECAS_EP_DEVOLUCAO);
    if (!Array.isArray(rawData)) return [];
    return (rawData as Record<string, unknown>[]).map(item => {
      if (item.data && typeof item.data === 'object') return { ...(item as unknown as VPecasEPecasRow), isDevolucao: true };
...
export async function saveVPecasEPecasDevolucaoRows(rows: VPecasEPecasRow[]): Promise<boolean> {
  try {
    await kvSet(KEY_VPECAS_EP_DEVOLUCAO, rows);
    return true;
  } catch {
    return false;
  }
...
export async function appendVPecasEPecasDevolucaoRows(
  periodo: string,
  newRows: Omit<VPecasEPecasRow, 'id'>[],
): Promise<{ added: number; removed: number }> {
  const existing = await loadVPecasEPecasDevolucaoRows();
  const kept = existing.filter(r => r.periodoImport !== periodo);
  const removed = existing.length - kept.length;
  const toAdd: VPecasEPecasRow[] = newRows.map(r => ({ ...r, id: crypto.randomUUID(), periodoImport: periodo, isDevolucao: true }));
  await saveVPecasEPecasDevolucaoRows([...kept, ...toAdd]);
...
// ─── Parser TXT ───────────────────────────────────────────────────────────────
export function parsePecasEPecasTxt(content: string): Omit<VPecasEPecasRow, 'id'>[] {
  ...
  const result: Omit<VPecasEPecasRow, 'id'>[] = [];
...
export function parsePecasEPecasDevolucaoTxt(content: string): Omit<VPecasEPecasRow, 'id'>[] {
...
// ─── Parser Excel ─────────────────────────────────────────────────────────────
export function parsePecasEPecasExcel(buffer: ArrayBuffer): Omit<VPecasEPecasRow, 'id'>[] {
