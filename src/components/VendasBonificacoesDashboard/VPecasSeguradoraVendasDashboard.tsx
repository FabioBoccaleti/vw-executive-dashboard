...
import { loadVPecasSegRows, loadVPecasSegDevolucaoRows } from './vPecasSeguradoraStorage';
import type { VPecasSegRow as VPecasRow } from './vPecasSeguradoraStorage';
...
const OV_KEY = 'vendas_pecas_seg_vendas_ov';
...
function EditCell({ value, type, onSave }: { value: string; type: 'text' | 'currency'; onSave: (v: string) => void }) {
    ...
    return (
      <div className="flex items-center gap-1 min-w-0">
        <input
          ...
          className="border border-sky-400 rounded px-1 py-0.5 text-xs w-full focus:outline-none focus:ring-1 focus:ring-sky-500"
        />
        <button onClick={commit} className="text-sky-600 hover:text-sky-700 flex-shrink-0"><Check className="w-3 h-3" /></button>
  ...
  return (
    <div
      className="group flex items-center justify-between gap-1 cursor-pointer hover:bg-sky-50 rounded px-1 py-0.5 min-w-0"
      onClick={() => { setDraft(value); setEditing(true); }}
    >
      <span className={`truncate text-xs ${type === 'currency' ? 'font-mono' : ''}`}>{display}</span>
      <Pencil className="w-2.5 h-2.5 text-slate-300 group-hover:text-sky-500 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
    </div>
  );
}
...
export default function VPecasSeguradoraVendasDashboard() {
  ...
  useEffect(() => {
    Promise.all([loadVPecasSegRows(), loadVPecasSegDevolucaoRows(), loadOverrides()]).then(([rows, devol, ov]) => {
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                const sheetName = `Peças Seg. Balcão - ${monthLabel}-${filterYear}`.slice(0, 31);
                await exportPecasExcel(filteredRows, overrides, sheetName, `vendas_pecas_seg_${filterYear}_${monthLabel}.xlsx`);
          ...
          <table className="w-full border-separate border-spacing-0 text-xs" style={{ minWidth: 2800 }}>
            <thead className="sticky top-0 z-10">
              ...
              <tr>
                ...
                <th colSpan={5}  className="bg-sky-800 text-white text-center py-1.5 text-[10px] font-semibold tracking-wide border-r border-sky-700">RECEITA BRUTA</th>
              ...
              <tr className="text-[10px] font-bold text-white">
                ...
                {['Valor Venda', 'ICMS', 'PIS', 'COFINS', 'Difal'].map((h, i) => (
                  <th key={i} className="bg-sky-800 px-3 py-2 text-right whitespace-nowrap border-b border-sky-700 border-r border-sky-700">{h}</th>
                ))}
                {/* REC. LÍQUIDA */}
              ...
              {filteredRows.length === 0 && (
                <tr>
                  <td colSpan={26} className="text-center py-16 text-slate-300 text-sm">
                    Nenhum registro — importe dados de Peças Seguradora Balcão na aba Registros
                  </td>
                </tr>
              )}
