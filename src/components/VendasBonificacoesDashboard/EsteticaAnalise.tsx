...
import { loadEsteticaDsr, type DsrConfig } from '@/components/CadastrosPage/cadastrosStorage';
...
function getRowYear(row: EsteticaRow): number {
  const date = row.dataEncerramento || row.dataRegistro;
  if (!date) return 0;
  return parseInt(date.split('-')[0]) || 0;
}
function getRowMonth(row: EsteticaRow): number {
  const date = row.dataEncerramento || row.dataRegistro;
  if (!date) return 0;
...
function validRows(rows: EsteticaRow[]): EsteticaRow[] {
  return rows.filter(r => SITUACOES_VALIDAS.has(r.situacao));
}

function filterRows(rows: EsteticaRow[], year: number | 'Todos', monthChip: number | null): EsteticaRow[] {
...
function calcMetrics(rows: EsteticaRow[], dsrList: DsrConfig[]): Metrics {
...
function filterByPeriod(rows: EsteticaRow[], slot: PeriodSlot): EsteticaRow[] {
...
// ─── Main Component ───────────────────────────────────────────────────────────
interface EsteticaAnaliseProps {
  rows: EsteticaRow[];
}

export function EsteticaAnalise({ rows }: EsteticaAnaliseProps) {
  ...
  useEffect(() => { loadEsteticaDsr().then(setDsrList); }, []);
  ...
  const periodMetrics = useMemo(
    () => periods.map(slot => {
      const byVendedor = (r: EsteticaRow) =>
