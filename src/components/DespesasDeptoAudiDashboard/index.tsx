...
import { printDespesasDeptoAudi } from './printDespesasDeptoAudi';
import {
  getDespesasAdmMes,
  setDespesasAdmMes,
...
export function DespesasDeptoAudiDashboard({ onChangeBrand }: Props) {
  ...
  async function handlePrint() {
    setPrinting(true);
    try {
      await printDespesasDeptoAudi(admYear, admMonth);
    } catch {
  ...
  useEffect(() => {
    kvKeys('despesas_depto_audi:*').then(keys => {
      const monthKeys = keys.filter(k => /^despesas_depto_audi:\d{4}:\d{2}$/.test(k));
      if (monthKeys.length === 0) return;
      const parsed = monthKeys.map(k => {
        const [, y, m] = k.split(':'); // despesas_depto_audi:2026:07 → ['despesas_depto_audi','2026','07']
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
      ...
      <header
        ...
        style={{ backgroundColor: '#bb0a30' }}
      >
        <div>
          <h1 className="text-sm font-bold text-white leading-tight">Despesas por Departamento na Sorana Audi</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{ backgroundColor: printing ? '#94a3b8' : '#bb0a30' }}
          >
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{ backgroundColor: importing ? '#94a3b8' : '#bb0a30' }}
            >
